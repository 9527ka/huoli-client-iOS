...
#import "WH_JXSelectFriendVC.h"
#import "WH_JXShare_WHViewController.h"
...
@interface WH_JXCustomShareVC () <JXShareVCDlegate, WH_JXSelectFriendVCDlegate>
...
@property (nonatomic, strong) WH_JXShareUser *user;
...
@implementation WH_JXCustomShareVC
...
#pragma mark - 发送给朋友
- (void)sendToFriend {
    WH_JXSelectFriendVC *selVC = [[WH_JXSelectFriendVC alloc] init];
    selVC.wh_delegate = self;
...
- (void)sendToFriendSuccess:(WH_JXSelectFriendVC *)selectVC user:(WH_JXShareUser *)user {
    ...
    if (self.url.length > 0) {
        int chatType = self.user.wh_roomId.length > 0 ? 2 : 1;
        
        [[WH_JXHttpRequet shareInstance] WH_sendMsgToUserId:self.user.wh_userId chatType:chatType type:1 content:self.url fileName:nil toView:self];
        return;
    }
    ...
    for (int i = 0; i < arr.count; i++) {
        if (self.imgArray.count > 0) {
            NSString *url = [[WH_JXHttpRequet shareInstance] WH_getDataUrlWithImage:arr[i]];
            [[WH_JXHttpRequet shareInstance] WH_uploadFile:url validTime:nil messageId:nil toView:self];
        }else if (self.videoArray.count > 0) {
            [[WH_JXHttpRequet shareInstance] WH_uploadFile:arr[i] validTime:nil messageId:nil toView:self];
        }
    }
}
...
- (void)shareLifeCircle {
    if (self.url.length > 0) {
        ...
        [self setProgressView];
        [self updateViewsIsComplete:NO title:nil content:nil];
        [[WH_JXHttpRequet shareInstance] WH_addMessage:self.url type:1 data:nil flag:3 toView:self];
        return;
    }

    WH_JXShare_WHViewController *shareVC = [[WH_JXShare_WHViewController alloc] init];
    shareVC.wh_delegate = self;
    shareVC.wh_image = self.imgV.image;
    [self presentViewController:shareVC animated:NO completion:nil];

}

- (void)WH_sendToLifeCircleSucces:(WH_JXShare_WHViewController *)shareVC {
    self.text = shareVC.wh_textView.text;
    self.proInt = 0;
    self.isSendToFriend = NO;
    [self setProgressView];
    [self updateViewsIsComplete:NO title:nil content:nil];
    
    NSString *path = [[WH_JXHttpRequet shareInstance] WH_getDataUrlWithImage:shareVC.wh_image];
    [[WH_JXHttpRequet shareInstance] WH_uploadFile:path validTime:nil messageId:nil toView:self];
}
...
- (void)getDataWithItem:(NSExtensionItem *)extensionItem {
    ...
    if([itemProvider hasItemConformingToTypeIdentifier:(NSString *)kUTTypeMovie]){
        [itemProvider loadItemForTypeIdentifier:(NSString *)kUTTypeMovie options:nil completionHandler:^(NSData *video, NSError *error) {
            ...
            if(video){
                dispatch_async(dispatch_get_main_queue(), ^{
                    NSString *path = [[WH_JXHttpRequet shareInstance] WH_getDataUrlWithVideo:video];
                    [self.videoArray addObject:path];

                    // 获取第一帧图片
                    UIImage *image = [[WH_JXHttpRequet shareInstance] WH_getFirstImageFromVideo:path];
                    // 图片加载完成后再创建UI， 避免出现无图片的空档时间
                    [self WH_setupViewsIsVideo:YES];
                    ...
                    [self.wh_tableBody addSubview:self.Playerview];
...
- (void)setHeadView {
    ...
    self.wh_tableBody = [[UIScrollView alloc] initWithFrame:CGRectMake(0, JX_SCREEN_TOP, JX_SCREEN_WIDTH, JX_SCREEN_HEIGHT-JX_SCREEN_TOP)];
    self.wh_tableBody.showsVerticalScrollIndicator = NO;
    self.wh_tableBody.showsHorizontalScrollIndicator = NO;
    [self.baseView addSubview:self.wh_tableBody];
...
//    [self.wh_tableBody addSubview:self.urlImageV];
    CGSize size = [url.absoluteString boundingRectWithSize:CGSizeMake(JX_SCREEN_WIDTH-140, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:sysFontWithSize(17)} context:nil].size;
    ...
    [self.wh_tableBody addSubview:self.urlTitle];

    _sendFBtn = [self WH_createMiXinButton:@"发送给朋友" drawTop:YES drawBottom:YES icon:@"im_linksShare_send_friend" click:@selector(sendToFriend)];
    _sendFBtn.frame = CGRectMake(MY_INSET,CGRectGetMaxY(_urlTitle.frame)+30, JX_SCREEN_WIDTH-MY_INSET*2, HEIGHT);
    _shareBtn = [self WH_createMiXinButton:@"分享给生活圈" drawTop:NO drawBottom:YES icon:@"im_linksShare_life" click:@selector(shareLifeCircle)];
    _shareBtn.frame = CGRectMake(MY_INSET,CGRectGetMaxY(_sendFBtn.frame), JX_SCREEN_WIDTH-MY_INSET*2, HEIGHT);
}


- (void)WH_setupViewsIsVideo:(BOOL)isVideo {
    [self setHeadView];
...
-(void) WH_didServerNetworkResultSucces:(WH_JXNetwork*)aDownload dict:(NSDictionary*)dict array:(NSArray*)array1{
    ...
    if ([aDownload.wh_action isEqualToString:wh_act_UploadFile]) {
        self.proInt += 1;
        if (self.isSendToFriend) {
            ...
            [[WH_JXHttpRequet shareInstance] WH_sendMsgToUserId:self.user.wh_userId chatType:chatType type:type content:url fileName:fileName toView:self];
        }else {
            //分享图片到生活圈
            [[WH_JXHttpRequet shareInstance] WH_addMessage:self.text type:2 data:dict flag:3 toView:self];
        }
    }
...
-(int) WH_didServerNetworkError:(WH_JXNetwork*)aDownload error:(NSError *)error{//error为空时，代表超时
    NSString *str = [NSString string];
    if ([aDownload.wh_action isEqualToString:act_SendMsg]) {
        str = @"消息发送失败";
    }
...
-(UIButton*)WH_createMiXinButton:(NSString*)title drawTop:(BOOL)drawTop drawBottom:(BOOL)drawBottom icon:(NSString*)icon click:(SEL)click{
