...
- (void)createContentViewWithFrame:(CGRect)frame {
    ...
    [headeImage sd_setImageWithURL:[NSURL URLWithString:_wh_headUrl?:@""] placeholderImage:[UIImage imageNamed:@"circleUser"]];
...
//    }
    ...
    NSString *timeStr = @"";
    if ([_wh_notice isKindOfClass:[NSDictionary class]]) {
        if (_wh_notice[@"time"]) {
            timeStr =  [self timestampToDate:[_wh_notice[@"time"] longLongValue]];
        }
    }
    timeLb.text = timeStr;
    ...
    [label setText:Localized(@"New_latest_announcement")];
    ...
    [bLabel setText:Localized(@"New_how_close_prompt")];
    ...
    NSArray *btnArray = @[@{@"btnImage":@"ignoreReminder" ,@"btnName":Localized(@"New_Ignore_reminder")} ,@{@"btnImage":@"intoGroup" ,@"btnName":Localized(@"New_enter_group")}];
    ...
    cTextView.text = _wh_notice[@"text"]?:@"";
...
- (void)questionMarkMethod {
    ...
    [mLabel setText:Localized(@"WH_EnterGroupCloseAlert")];
