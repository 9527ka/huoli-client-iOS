...
@implementation WH_JXCommonService
...
- (void)WH_sendCourse:(WH_JXMsgAndUserObject *)obj Array:(NSArray *)array {
    ...
    self.sendLabel.font = sysFontWithSize(12);
    ...
    _sendIndex = 0;
    _timeIndex = 0;
    _wh_courseTimer = [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(sendCourseTimerAction:) userInfo:obj repeats:YES];
...
- (void)sendCourseTimerAction:(NSTimer *)timer {
    ...
    _timeIndex ++;
    WH_JXMessageObject *msg= _courseArray[self.sendIndex];
    ...
    [g_notify postNotificationName:kSendCourseMsg_WHNotification object:msg];
    
    self.sendIndex ++;
    
    if (_courseArray.count == self.sendIndex) {
        [_wh_courseTimer invalidate];
        _wh_courseTimer = nil;
