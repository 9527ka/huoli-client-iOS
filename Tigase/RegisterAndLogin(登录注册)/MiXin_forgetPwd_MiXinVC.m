...
@interface MiXin_forgetPwd_MiXinVC () <UITextFieldDelegate>
{
...
@implementation MiXin_forgetPwd_MiXinVC
...
//   int n = 12;
   ...
   UIView *tView = [self createViewWithOrginY:12 viewWidth:JX_SCREEN_WIDTH - 2*g_factory.globelEdgeInset viewHeight:(!self.isModify)?HEIGHT*2:HEIGHT];
   //区号
   if (!_phone) {
      NSString *str = @"";
      if (self.forgetType == 1) {
         str = Localized(@"JX_InputUserAccount");
      }else{
         str = Localized(@"JX_InputPhone");
      }
      _phone = [UIFactory WH_create_WHTextFieldWith:CGRectMake(16, 0, tView.frame.size.width - 32, HEIGHT) delegate:self returnKeyType:UIReturnKeyNext secureTextEntry:NO placeholder:str font:[UIFont fontWithName:@"PingFangSC-Regular" size: 15]];
      [tView addSubview:_phone];
      
//        if (self.forgetType == 0) {
//         ////使用手机号
...
//        }
   ...
   if (!self.isModify) {
      ...
      //图片验证码
      _imgCode = [UIFactory WH_create_WHTextFieldWith:CGRectMake(16, HEIGHT, tView.width -70-12-16-10, HEIGHT) delegate:self returnKeyType:UIReturnKeyNext secureTextEntry:NO placeholder:Localized(@"JX_inputImgCode") font:[UIFont fontWithName:@"PingFangSC-Regular" size: 15]];
      ...
      _graphicButton.frame = CGRectMake(_imgCode.right + 12, (HEIGHT - 35)/2, 70, 35);
      ...
      _code = [[UITextField alloc] initWithFrame:CGRectMake(16, 0, cView.frame.size.width-32, HEIGHT)];
      _code.delegate = self;
...
//      n = n+HEIGHT+INSETS;
   ...
   UIView *bView = [self createViewWithOrginY:height + 12 viewWidth:JX_SCREEN_WIDTH - 2*g_factory.globelEdgeInset viewHeight:(self.isModify)?HEIGHT*3:HEIGHT*2];
   
   CGFloat pHight = 0;
   if (self.isModify) {
      _oldPwd = [[UITextField alloc] initWithFrame:CGRectMake(16,0,CGRectGetWidth(bView.frame)-32,HEIGHT)];
...
//      _oldPwd.borderStyle = UITextBorderStyleRoundedRect;
   ...
   _pwd = [[UITextField alloc] initWithFrame:CGRectMake(16, pHight, bView.width-32,HEIGHT)];
   _pwd.delegate = self;
...
//   _pwd.borderStyle = UITextBorderStyleRoundedRect;
   ...
   _repeat = [[UITextField alloc] initWithFrame:CGRectMake(16, CGRectGetMaxY(_pwd.frame), bView.width - 32,HEIGHT)];
...
//   [self createLeftViewWithImage:[UIImage imageNamed:@"password"] superView:_repeat];
   ...
   UIButton* _btn = [UIFactory WH_create_WHCommonButton:Localized(@"JX_UpdatePassWord") target:self action:@selector(changePassword:)];
   ...
   if (self.isModify) {
      if ([g_config.lastLoginType intValue] == 1) {
         _phone.text = [g_default objectForKey:kMY_USER_LoginName];
      }else {
         _phone.text = [g_myself.telephone containsString:areaStr] ? [g_myself.telephone substringFromIndex:areaStr.length] : g_myself.telephone;
      }
   }
   if (self.isModify) {
      _phone.enabled = NO;
...
//   NSString *areaCode = [_areaCodeBtn.titleLabel.text stringByReplacingOccurrencesOfString:@"+" withString:@""];
   ...
   if (self.isModify){
     ...
     BOOL smsCodeRight = [_code.text isEqualToString:_smsCode];

      if(smsCodeRight){
         [_wait start];
//         [g_server resetPwd:phoneString areaCode:areaCode randcode:_smsCode newPwd:_pwd.text toView:self];
         [g_server resetPwd:_phone.text areaCode:areaCode randcode:_smsCode newPwd:_pwd.text registerType:self.forgetType toView:self];
      } else {
...
- (BOOL)isMobileNumber:(NSString *)number{
   if ([_phone.text length] == 0) {
    ...
    if (!isMatch) {
       [g_App showAlert:Localized(@"MiXin_inputPhone_MiXinVC_InputTurePhone")];
       return NO;
    }
    return YES;
}
...
-(int) WH_didServerResult_WHFailed:(WH_JXConnection*)aDownload dict:(NSDictionary*)dict{
   if([aDownload.action isEqualToString:wh_act_SendSMS]){
      ...
      [GKMessageTool showText:[NSString stringWithFormat:@"%@",error]];
      
      return WH_hide_error;
   }else if ([aDownload.action isEqualToString:wh_act_PwdReset]) {
      if ([dict[@"resultCode"] integerValue] == 0) {
         [GKMessageTool showText:dict[@"resultMsg"]];
      }else {
         [GKMessageTool showText:dict[@"data"][@"resultMsg"]];
      }
