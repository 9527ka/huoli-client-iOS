...
@interface MiXin_inputPhone_MiXinVC ()<UITextFieldDelegate>
{
    NSTimer *_timer;
...
@implementation MiXin_inputPhone_MiXinVC
...
//        [imCView addSubview:imCIView];
        ...
        [goLoginBtn addTarget:self action:@selector(goToMiXin_login_MiXinVC) forControlEvents:UIControlEventTouchUpInside];
        ...
        //弃用手机验证
        //#ifdef IS_TEST_VERSION
        //        UIButton* _btn = [UIFactory MiXin_create_MiXinCommonButton:@"下一步" target:self action:@selector(onTest)];
        //#else
        //        UIButton* _btn = [UIFactory MiXin_create_MiXinCommonButton:@"下一步" target:self action:@selector(onClick)];
        //#endif
        //新添加的手机验证（注册）
        n = n+HEIGHT+INSETS;
//        [UIFactory MiXin_create_MiXinCommonButton:Localized(@"REGISTERS") target:self action:@selector(checkPhoneNumber)];
...
//        careTitle.text = Localized(@"MiXin_inputPhone_MiXinVC_BeCareful");
//
//        n = n+HEIGHT+INSETS;
...
//        careFirst.text = Localized(@"MiXin_inputPhone_MiXinVC_NotNeedCode");
//
//        n = n+HEIGHT/3+INSETS;
...
//        careSecond.text = Localized(@"MiXin_inputPhone_MiXinVC_NoReg");
...
- (void)checkPhoneNumber{
    ...
    if (isMobile) {
//        [g_server checkPhone:_phone.text toView:self];
...
-(void)onClick{
    ...
    if (!_isSkipSMS) {
        ...
        if([_smsCode length]<6){
            [g_App showAlert:Localized(@"MiXin_inputPhone_MiXinVC_NoMegCode")];
            return;
        }
...
- (void)setUserInfo {
    if (_agreeImgV.isHidden == YES) {
        [GKMessageTool showTips:Localized(@"JX_NotAgreeProtocol")];
        return;
    }
...
//    MiXin_inputPwd_MiXinVC* vc = [MiXin_inputPwd_MiXinVC alloc];
...
//    [self.view endEditing:YES];
//
//    MiXin_JXTelAreaList_MiXinVC *telAreaListVC = [[MiXin_JXTelAreaList_MiXinVC alloc] init];
...
-(void)onSend{
    if (!_send.selected) {
        ...
        [g_server WH_sendSMSCodeWithTel:_phone.text areaCode:areaCode isRegister:NO imgCode:_imgCode.text toView:self];
