...
-(void)roomHeadImageToView:(UIImageView *)toView{
    ...
    if (toView){
        ...
        if (MainHeadType) {//圆形
            ...
            toView.image = [UIImage imageNamed:@"fangxinggroupImagePlaceholder"];//先设置一张默认群组图片
        }
    }
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        for (NSNumber * userIdNum in userIdArr) {
            ...
            [manager loadImageWithURL:[NSURL URLWithString:url] options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
                ...
                if (downLoadImageArr.count >= count || finishCount >= userIdArr.count){
...
-(NSString *)roomDataToNSString{
    ...
    return [[self toDictionary] mj_JSONString];
}
...
-(void)WH_getDataFromDict:(NSDictionary*)dict{
    ...
    self.allowForceNotice = [[dict objectForKey:@"allowForceNotice"] boolValue];
...
//    self.call = [dict objectForKey:@"call"];
    ...
    self.isShowSignIn = [[dict objectForKey:@"isShowSignIn"] integerValue];
    ...
    for(int i=0;i<[array count];i++){
        ...
        [option insert];
//        Boolean isInsert = [option insert];
//        if (!isInsert) {
//            [option update];
//        }
        p = nil;
...
-(void)WH_getDataFromDict:(NSDictionary*)dict{
    ...
    self.vip = [NSNumber numberWithInteger:[[dict objectForKey:@"vip"] integerValue]];
    
//    self.user = [dict objectForKey:@"user"];
    self.onLineState = [NSNumber numberWithInteger:[[dict objectForKey:@"onLineState"] integerValue]];
...
-(BOOL)checkTableCreatedInDb:(NSString *)queryRoomId{
    ...
    NSString *createStr=[NSString stringWithFormat:@"CREATE  TABLE IF NOT EXISTS 'member_%@' ('userId' INTEGER PRIMARY KEY NOT NULL  UNIQUE , 'roomId' VARCHAR, 'userName' VARCHAR, 'cardName' VARCHAR, 'role' INTEGER, 'createTime' VARCHAR, 'remarkName' VARCHAR,'vip' INTEGER,'active' INTEGER,'offlineTime' INTEGER,'onLineState' INTEGER,'isAddFirend' INTEGER)",queryRoomId];
...
//    }
    ...
    if (!worked) {
//        [self update];
        NSLog(@"插入member_%@数据失败",self.roomId);
    }
    db = nil;
    return worked;
}
-(BOOL)update{
    ...
    NSString* sql = [NSString stringWithFormat:@"update member_%@ set roomId=?,userId=?,userName=?,cardName=?,role=?,createTime=?,remarkName=?,vip=?,active=?,offlineTime=?,onLineState=?,isAddFirend=? where userId=?",self.roomId];
    BOOL worked = [db executeUpdate:sql,self.roomId,[NSNumber numberWithLong:self.userId],self.userNickName,self.cardName,self.role,[NSNumber numberWithLongLong:self.createTime],self.lordRemarkName,self.vip,[NSNumber numberWithLongLong:self.active],[NSNumber numberWithLongLong:self.offlineTime],self.onLineState,self.isAddFirend,[NSNumber numberWithLong:self.userId]];
