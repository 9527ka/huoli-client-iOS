...
+ (instancetype)sharedConstant {
    static WH_JXConstant *sharedConstant = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedConstant = [[WH_JXConstant alloc] init];
    });
...
-(id)init{
    ...
    _isAddFriend = [NSNumber numberWithBool:NO];
    ...
    _chatBackgrounImage = [NSMutableDictionary dictionary];
...
- (void) getData {
    ...
    self.chatBackgrounImage = [self getChatBackgrounImage];
...
- (NSMutableDictionary *)getChatBackgrounImage {
    NSString *path = ChatBackgroundImage;
    NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithContentsOfFile:path];
    if (!dict) {
        dict = [NSMutableDictionary dictionary];
    }
...
//        [dict setObject:[rs objectForColumnName:@"th"] forKey:@"th"];
        ...
        [dict setObject:[rs objectForColumnName:@"big6"] forKey:@"big6"];
        [dict setObject:[rs objectForColumnName:@"big7"] forKey:@"big7"];
...
-(NSString*)getCurCountryFieldName{
    ...
    }else if ([_sysLanguage isEqualToString:@"big6"]) {
        
        name = [[NSString alloc] initWithFormat:@"%@",ALABOYU];
    }else if ([_sysLanguage isEqualToString:@"big7"]) {
        
        name = [[NSString alloc] initWithFormat:@"%@",FAYU];
