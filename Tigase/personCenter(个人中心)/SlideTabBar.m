...
-(void)layoutSubviews {
    ...
    CGFloat itemWidth = _itemWidth = (JX_SCREEN_WIDTH - 40)/_titles.count;
    [_titles enumerateObjectsUsingBlock:^(NSString * title, NSUInteger idx, BOOL *stop) {
        UILabel *label = [[UILabel alloc]init];
        label.text = title;
        label.textColor = HEXCOLOR(0xBABABA);
...
//            spliteLine.backgroundColor = THEMECOLOR;
            spliteLine.layer.zPosition = 10;
            [self addSubview:spliteLine];
        }
    }];
    _labels[_tabIndex].textColor = HEXCOLOR(0x161819);
    
    _slideLightView = [[UIView alloc] init];
    _slideLightView.backgroundColor = THEMECOLOR;
    _slideLightView.frame = CGRectMake(_tabIndex * itemWidth + (itemWidth - 18)/2, self.bounds.size.height-2, 18, 2);
...
//                             frame.origin.x = self.itemWidth * index + 15;
            frame.origin.x = index * self.itemWidth + (self.itemWidth - 18)/2;
            
                             [self.slideLightView setFrame:frame];
                             [self.labels enumerateObjectsUsingBlock:^(UILabel *label, NSUInteger idx, BOOL *stop) {
                                 label.textColor = index == idx ? HEXCOLOR(0x161819) : HEXCOLOR(0xBABABA);
                             }];
