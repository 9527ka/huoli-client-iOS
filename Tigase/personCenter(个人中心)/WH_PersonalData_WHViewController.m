...
- (void)viewDidLoad {
    ...
    self.title = Localized(@"New_personal_information");
...
- (void)createContentView {
    ...
    if ([g_config.regeditPhoneOrName intValue] != 1) {
        // 0：使用手机号注册，1：使用用户名注册
        tStr = Localized(@"JX_MobilePhoneNo.");
    }else{
...
//        }
        ...
        for (int i = 0; i < nameArray.count; i++) {
            ...
            if ([currentTitleString isEqualToString:Localized(@"JX_NickName")]) {//昵称
                ...
                self.wh_sex = [[UISegmentedControl alloc] initWithItems:[NSArray arrayWithObjects:Localized(@"JX_Wuman"),Localized(@"JX_Man"),nil]];
                self.wh_sex.frame = CGRectMake(self.wh_cTableView.frame.size.width - 92 ,(55 - 30)/2,80,30);
                
                NSDictionary* selectedTextAttributes =@{NSFontAttributeName:sysFontWithSize(15),NSForegroundColorAttributeName:[UIColor whiteColor]};
                [self.wh_sex setTitleTextAttributes:selectedTextAttributes forState:UIControlStateSelected];//设置文字属性
                NSDictionary* unselectedTextAttributes =@{NSFontAttributeName:sysFontWithSize(15),NSForegroundColorAttributeName:HEXCOLOR(0x969696)};
                [self.wh_sex setTitleTextAttributes:unselectedTextAttributes forState:UIControlStateNormal];
