    ...
    if (self.viewControllers.count > 0) {
        if (self.visibleViewController.gk_backStyle != GKNavigationBarBackStyleNone) {
            ...
            if ([viewController isKindOfClass:[GKNavigationBarViewController class]]) {
                ...
                UIImage *image = self.visibleViewController.gk_backStyle == GKNavigationBarBackStyleBlack ? GKImage(@"btn_back_black") : GKImage(@"btn_back_white");
                
                if (vc.gk_navLeftBarButtonItem == nil && vc.gk_navLeftBarButtonItems == nil) {
                    vc.gk_navLeftBarButtonItem = [UIBarButtonItem itemWithTitle:nil image:image target:self action:@selector(goBack)];
                }
            }
        }
    }
