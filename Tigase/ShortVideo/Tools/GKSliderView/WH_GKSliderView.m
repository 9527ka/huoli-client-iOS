...
@implementation WH_GKSliderView
...
- (void)sliderBtnTouchBegin:(UIButton *)btn {
    ...
    if ([self.delegate respondsToSelector:@selector(WH_sliderTouchBegan:)]) {
        [self.delegate WH_sliderTouchBegan:self.value];
    }
}

- (void)sliderBtnTouchEnded:(UIButton *)btn {
    
    if ([self.delegate respondsToSelector:@selector(WH_sliderTouchEnded:)]) {
        [self.delegate WH_sliderTouchEnded:self.value];
    }
}
...
- (void)sliderBtnDragMoving:(UIButton *)btn event:(UIEvent *)event {
    ...
    if ([self.delegate respondsToSelector:@selector(WH_sliderValueChanged:)]) {
        [self.delegate WH_sliderValueChanged:value];
    }
}
...
- (void)tapped:(UITapGestureRecognizer *)tap {
    ...
    if ([self.delegate respondsToSelector:@selector(WH_sliderTapped:)]) {
        [self.delegate WH_sliderTapped:value];
    }
}
