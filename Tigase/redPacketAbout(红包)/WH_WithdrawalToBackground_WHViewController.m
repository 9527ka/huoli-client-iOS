...
- (void)viewDidLoad {
    ...
    UIView *bView = [self createBottomViewWithOrginY:CGRectGetMaxY(tView.frame) + 12];
...
- (void)createContentTable {
    ...
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(g_factory.globelEdgeInset, 300, JX_SCREEN_WIDTH - 2*g_factory.globelEdgeInset, 60)];
    ...
    self.usdtTextField = [[UITextField alloc] initWithFrame:CGRectMake(20, 0, view.frame.size.width-40, 60)];
    [self.usdtTextField setTextColor:HEXCOLOR(0x3A404C)];
    [self.usdtTextField setFont:sysBoldFontWithSize(16)];
    ...
    [view addSubview:self.usdtTextField];
...
- (UIView *)createMoneyView {
    UIView *cView = [self createViewWithRect:CGRectMake(0, 0, JX_SCREEN_WIDTH, 184 + 24 + 100) backgroundColor:self.wh_tableBody.backgroundColor];
    ...
    UILabel *pmLabel = [UIFactory WH_create_WHLabelWith:CGRectMake(20, CGRectGetMaxY(label.frame) + 20, 80, 63) text:@"HOTC"];
    ...
    [self.moneyTextField setPlaceholder:Localized(@"New_please_enter_amount")];
...
//    CGRectMake(g_factory.globelEdgeInset, JX_SCREEN_HEIGHT - (168 + 15 + 44), CGRectGetWidth(self.pMementTable.frame) - 2*g_factory.globelEdgeInset, 168 + 15 + 44)
    ...
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(g_factory.globelEdgeInset, 300, JX_SCREEN_WIDTH - 2*g_factory.globelEdgeInset, 168 + 15 + 44)];
    ...
    [confirmBtn setTitle:Localized(@"New_confirm_withdrawal") forState:UIControlStateNormal];
...
//    [label setLineBreakMode:NSLineBreakB  yWordWrapping];
    ...
    if (IS_CanAdd_WithdrawAccount) {//可以添加提现账号
        ...
        if (self.checkIndex == indexPath.row) {
            ...
            if (self.checkIndex == 0) {
                ...
                array = @[Localized(@"New_please_enter_bank_card_number") ,@"请输入银行账户名",@"请输入开户行"];
            }
...
- (void)confirmWithdrawalMethod {
    ...
    if (countStr.length == 0) {
        [GKMessageTool showText:Localized(@"New_please_enther_withdrawal_amount")];
        return;
    }
    ...
    if ([countStr doubleValue] > [purseMoney doubleValue]) {
        [GKMessageTool showText:Localized(@"New_your_credit_is_running_low")];
        return;
    }
...
//        self.isAlipay = button.tag == 1000;
        ...
        self.verVC.usdlUrl = self.usdtTextField.text;
...
//            return;
        ...
        [g_server userWithdrawalWithUserId:g_myself.userId amount:[NSString stringWithFormat:@"%@",[NSNumber numberWithDouble:[self.moneyTextField.text doubleValue]]] secret:secret context:contextStr accountType:type  usdlUrl:self.usdtTextField.text.length?self.usdtTextField.text:@"" toView:self time:time];
    } else {
        self.listArray = [[NSMutableArray alloc] initWithObjects:@{@"icon":@"MX_MyWallet_Alipay" ,@"name":@"支付宝"} ,@{@"icon":@"WH_BankIcon" ,@"name":@"银行卡支付"} , nil];
        NSString *nameStr = [self.nameTextField.text stringByReplacingOccurrencesOfString:@" " withString:@""];
        NSString *codeStr = [self.codeTextField.text stringByReplacingOccurrencesOfString:@" " withString:@""];
...
-(int) WH_didServerResult_WHFailed:(WH_JXConnection*)aDownload dict:(NSDictionary*)dict {
    [_wait stop];
    if (![aDownload.action isEqualToString:wh_act_userWithdrawMethodGet]) {
        [g_App showAlert:Localized(@"New_withdrawal_failed_please_try_agin")];
    }
