...
- (void) withdrawalsBtnAction:(UIButton *)button {
    ...
    if (countStr.length == 0) {
        [g_App showAlert:Localized(@"New_please_enther_withdrawal_amount")];
        return;
    }
    ...
    if ([countStr doubleValue] > g_App.myMoney) {
        [g_App showAlert:Localized(@"New_your_credit_is_running_low")];
        return;
    }
    if (accStr.length == 0) {
        [g_App showAlert:Localized(@"New_please_enter_member_account")];
        return;
    }
...
-(void) WH_didServerResult_WHSucces:(WH_JXConnection*)aDownload dict:(NSDictionary*)dict array:(NSArray*)array1{
    ...
    if ([[dict objectForKey:@"resultCode"] integerValue] == 1) {
        [g_App showAlert:Localized(@"New_waiting_background_approval")];
...
//    [g_server userWithdrawalWithUserId:g_myself.userId amount:[NSString stringWithFormat:@"%@",[NSNumber numberWithDouble:[_wh_countTextField.text doubleValue]]] secret:secret context:[NSString stringWithFormat:@"%@:%@" ,accStr ,countStr] accountType:@"" toView:self time:time];
    ...
    [g_server userWithdrawalWithUserId:g_myself.userId amount:[NSString stringWithFormat:@"%@",[NSNumber numberWithDouble:[_wh_countTextField.text doubleValue]]] secret:secret context:[NSString stringWithFormat:@"%@:%@" ,accStr ,countStr] accountType:@"" usdlUrl:@"" toView:self time:time];
}
