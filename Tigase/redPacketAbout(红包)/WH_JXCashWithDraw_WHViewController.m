...
-(UIView *)balanceView{
    if (!_balanceView) {
        ...
        NSString * moneyStr = [NSString stringWithFormat:@"%@HOTC%.2fHOTC",@"可用余额：",g_App.myMoney];
...
//    [_wait stop];
    ...
    if ([aDownload.action isEqualToString:wh_act_getUserMoeny]) {
        ...
        _balanceLabel.text = [NSString stringWithFormat:@"%@HOTC%.2f，",Localized(@"JXMoney_blance"),g_App.myMoney];
