...
- (void)replyToTheRedPacket {
    ...
    self.replayTitle.text = Localized(@"New_reply_width_word");
    
    self.replayTextView.textColor = self.watermarkColor;
    self.replayTextView.text = self.replyContent.length > 0 ? self.replyContent : Localized(@"New_reply_width_word");
...
- (void)textViewDidChange:(UITextView *)textView {
    ...
    if ([textView.text isEqualToString:@""]) {
        ...
        textView.text = self.replyContent.length > 0 ? self.replyContent : Localized(@"New_reply_width_word");
    }
...
//            [self WH_creatTBHeaderView];
        ...
        if (IS_SHOW_EXCLUSIVEREDPACKET) {
            isCanOpen = Localized(@"New_not_finished");
        }else{
