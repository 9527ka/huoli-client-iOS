...
//        [sv release];
        ...
        _iv = [[WH_JXImageView alloc]initWithFrame:CGRectMake(JX_SCREEN_WIDTH-41,0,41,41)];
        _iv.image = [UIImage imageNamed:@"playback_close"];
        _iv.didTouch = @selector(actionQuit);
        _iv.wh_delegate = self;
...
-(void)showImages{
    _photoCount = (int)[wh_photos count];
    if(_photoCount<=0)
        return;
...
//        [lb release];
        ...
        WH_JXImageView* iv = [[WH_JXImageView alloc]initWithFrame:CGRectMake(0, 0, sv.frame.size.width, sv.frame.size.height)];
...
//        [iv release];
        ...
        NSString* url = [[wh_photos objectAtIndex:i] objectForKey:@"oUrl"];
        [g_server WH_getImageWithUrl:url imageView:iv];
    }
}
...
- (void)setPage
{   
	sv.contentOffset = CGPointMake(JX_SCREEN_WIDTH*_wh_page, 0.0f);
    NSString* url = [[wh_photos objectAtIndex:_wh_page] objectForKey:@"oUrl"];
    [g_server WH_getImageWithUrl:url imageView:[_array objectAtIndex:_wh_page]];
}

- (void)setWh_page:(int)value{
    _wh_page = value;
    if(_wh_page<0)
        _wh_page=0;
    if(_wh_page >= _photoCount)
        _wh_page = 0;
    [self setPage];
}
...
-(void) doReadNext{
    if(_wh_page<_photoCount-1){
        NSString* url = [[wh_photos objectAtIndex:_wh_page+1] objectForKey:@"oUrl"];
        [g_server WH_getImageWithUrl:url imageView:[_array objectAtIndex:_wh_page+1]];
    }
}
...
- (UIView *)viewForZoomingInScrollView:(UIScrollView *)scrollView{
    if(scrollView.tag == 1)
        return nil;
    return [_array objectAtIndex:_wh_page];
}
