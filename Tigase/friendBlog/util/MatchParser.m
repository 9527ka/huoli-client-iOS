...
-(void)match:(NSString *)source   atCallBack:(BOOL (^)(NSString *))atString   title:(NSAttributedString *)title1   link:(void(^)(NSMutableAttributedString*attrString,NSRange range))link
{
    ...
    _source=source;
    source=[NSStrUtil trimString:source];
    if ([source containsString:@"\r"]) {//过滤换行符
    ...
    [self matchLink:text attrString:attStr offset:(int)offset link:link];
    CTParagraphStyleSetting lineBreakMode;
    CTLineBreakMode lineBreak = kCTLineBreakByCharWrapping;
    lineBreakMode.spec = kCTParagraphStyleSpecifierLineBreakMode;
    lineBreakMode.value = &lineBreak;
...
#pragma -mark 获取内容行数 以及 一行时，内容的宽度
    // 获取内容行数 以及 一行时，内容的宽度
    _numberOfTotalLines=(int)[lines count];
...
//            CTLineRef line1 = (__bridge CTLineRef)lines[i];
//            NSArray *runs = (__bridge NSArray *)CTLineGetGlyphRuns(line1);
