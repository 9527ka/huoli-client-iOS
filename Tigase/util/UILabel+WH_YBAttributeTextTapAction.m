//
//  UILabel+WH_YBAttributeTextTapAction.m
//
...
@interface WH_YBAttributeModel : NSObject
...
@implementation WH_YBAttributeModel

@end

@implementation UILabel (WH_YBAttributeTextTapAction)
...
- (id<WH_YBAttributeTapActionDelegate>)delegate
{
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setDelegate:(id<WH_YBAttributeTapActionDelegate>)delegate
{
    objc_setAssociatedObject(self, @selector(wh_delegate), delegate, OBJC_ASSOCIATION_ASSIGN);
}
...
- (void)yb_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings
                                   delegate:(id <WH_YBAttributeTapActionDelegate> )delegate
{
    [self yb_getRangesWithStrings:strings];
    self.userInteractionEnabled = YES;
...
- (void)yb_addAttributeTapActionWithRanges:(NSArray<NSString *> *)ranges delegate:(id<WH_YBAttributeTapActionDelegate>)delegate
{
    [self yb_getRangesWithRanges:ranges];
    self.userInteractionEnabled = YES;
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            for (int j = 0; j < link_count; j++) {
                ...
                WH_YBAttributeModel *model = self.attributeStrings[j];
...
- (void)yb_getRangesWithStrings:(NSArray <NSString *>  *)strings
{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            WH_YBAttributeModel *model = [WH_YBAttributeModel new];
            model.range = range;
            model.str = obj;
