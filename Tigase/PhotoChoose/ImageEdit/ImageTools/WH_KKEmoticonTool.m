...
#import "WH_KKEmoticonTool.h"
#import "WH_KKToolBarItem.h"
#import "WH_KKEmoticonView.h"

@implementation WH_KKEmoticonTool{
    //修改前的图片
...
+(NSString *)defaultTitle{
    return Localized(@"JX_ImageEditAddImage");
}
...
{
    [WH_KKEmoticonView setActiveEmoticonView:nil];
...
- (void)setEmoticonMenu
{
    ...
    for(NSString *imgNmae in list){
        ...
        if(image){
            WH_KKToolBarItem *view = [[WH_KKToolBarItem alloc] initWithFrame:CGRectMake(x, 0, W, H) target:self action:@selector(tappedEmoticonPanel:) toolInfo:nil];
...
{
    WH_KKToolBarItem *kkview = (WH_KKToolBarItem *)sender.view;
    
    WH_KKEmoticonView *view = [[WH_KKEmoticonView alloc] initWithImage:kkview.iconView.image tool:self];
...
//    [view setScale:ratio];
    ...
    [WH_KKEmoticonView setActiveEmoticonView:view];
