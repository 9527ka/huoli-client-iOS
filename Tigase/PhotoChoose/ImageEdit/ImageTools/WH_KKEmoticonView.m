...
{
    static WH_KKEmoticonView *activeView = nil;
    if(view != activeView){
...
- (instancetype)initWithImage:(UIImage *)image tool:(WH_KKEmoticonTool *)tool{
    ...
    if (self) {
        ...
        _scaleBtn = [[WH_KKScaleButton alloc] initWithFrame:CGRectMake(0, 0, kDeleteBtnSize, kDeleteBtnSize)];
...
{
    WH_KKEmoticonView *nextTarget = nil;
    
    const NSInteger index = [self.superview.subviews indexOfObject:self];
    
    for(NSInteger i=index+1; i<self.superview.subviews.count; ++i){
        UIView *view = [self.superview.subviews objectAtIndex:i];
        if([view isKindOfClass:[WH_KKEmoticonView class]]){
            nextTarget = (WH_KKEmoticonView *)view;
            break;
        }
    }
