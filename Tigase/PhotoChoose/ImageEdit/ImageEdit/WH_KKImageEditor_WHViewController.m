...
@interface WH_KKImageEditor_WHViewController ()<WH_KKImageToolProtocol>

@property (nonatomic, strong) WH_KKImageToolBase *currentTool;
@property (nonatomic, strong) WH_KKImageToolInfo *toolInfo;

@end

@implementation WH_KKImageEditor_WHViewController{
...
- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        self.toolInfo = [WH_KKImageToolInfo toolInfoForToolClass:[self class]];
    }
    return self;
}
...
- (void)viewDidLoad{
    ...
    [self.view setBackgroundColor:[WH_KKImageEditorTheme theme].backgroundColor];
...
- (void)initMenuView{
    if (_menuView == nil) {
        ...
        _menuView.backgroundColor = [WH_KKImageEditorTheme theme].toolbarColor;
...
- (void)initToolSettings{
    ...
    for(WH_KKImageToolInfo *info in [WH_KKImageToolInfo sortWithTools:self.toolInfo.subtools]){
        if ([info.title isEqualToString:[WH_KKImageToolBase defaultTitle]]) {
            continue;
        }
        WH_KKToolBarItem *view = [[WH_KKToolBarItem alloc] initWithFrame:CGRectMake(x+padding, 0, W, H) target:self action:@selector(tappedToolMenuItem:) toolInfo:info];
...
- (void)initNavigationBar{
    NSString *title;
    if ([self.delegate isKindOfClass:[WH_JXChat_WHViewController class]] || [self.delegate isKindOfClass:[WH_JXCamera_WHVC class]]) {
...
+ (NSArray*)subtools{
    return [WH_KKImageToolInfo toolsWithToolClass:[WH_KKImageToolBase class]];
}
...
{
    WH_KKToolBarItem *view = (WH_KKToolBarItem *)sender.view;
...
//设置当前工具信息
- (void)setupToolWithToolInfo:(WH_KKImageToolInfo*)info
{
    ...
    if(toolClass){
        ...
        if(instance!=nil && [instance isKindOfClass:[WH_KKImageToolBase class]]){
...
//初始化当前工具
- (void)setCurrentTool:(WH_KKImageToolBase *)currentTool
{
