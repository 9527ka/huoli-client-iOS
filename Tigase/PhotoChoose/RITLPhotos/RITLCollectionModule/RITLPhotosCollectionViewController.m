...
//        iv = [[UIImage imageNamed:@"navBarBackground"] imageWithTintColor:HEXCOLOR(0x00ceb3)];
//    }else {
...
//    [self.navigationController.navigationBar setBackgroundImage:iv forBarMetrics:UIBarMetricsDefault];
    ...
    [self.navigationController.navigationBar setBackgroundColor:HEXCOLOR(0xffffff)];
    [self.navigationController.navigationBar setTintColor:HEXCOLOR(0x333333)];
...
- (void)photosCellDidTouchUpInSlide:(RITLPhotosCell *)cell asset:(PHAsset *)asset indexPath:(NSIndexPath *)indexPath complete:(RITLPhotosCellStatusAction)animated
{
    ...
    NSInteger index = 0;
    if (asset) {
        index = [self.dataManager addOrRemoveAsset:asset].integerValue;
    }
