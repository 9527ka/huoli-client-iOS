...
@interface RITLPhotosHorBrowseViewController ()<UICollectionViewDelegate,UINavigationControllerDelegate,KKImageEditorDelegate>
...
@property (nonatomic, strong) RITLPhotosBrowseImageCell *cell;  // 记录一下编辑的cell
...
//        view.backgroundColor = THEMECOLOR;
    ...
    // 记录当前控制器的cell.indexPath.row
    self.cellRow = self.dataSource.defaultItemIndexPath.row;
...
- (void)updateTopViewWithAsset:(PHAsset *)asset
{
    ...
    if (!configuration.containImage) {  //如果不包含图片
        
        self.statusButton.hidden = (asset.mediaType == PHAssetMediaTypeImage);
    }

    //进行属性隐藏设置
    self.indexLabel.hidden = !isSelected;
    //视频隐藏编辑功能
    self.bottomView.previewButton.hidden = (asset.mediaType == PHAssetMediaTypeVideo);
...
- (void)editImage {
    self.isEdit = NO;
    self.cell = (RITLPhotosBrowseImageCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:self.cellRow inSection:0]];
    WH_KKImageEditor_WHViewController *editor = [[WH_KKImageEditor_WHViewController alloc] initWithImage:self.cell.imageView.image delegate:self];
