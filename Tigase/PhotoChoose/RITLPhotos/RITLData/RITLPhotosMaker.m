            ...
            [self.imageManager requestAVAssetForVideo:asset options:options resultHandler:^(AVAsset * _Nullable asset, AVAudioMix * _Nullable audioMix, NSDictionary * _Nullable info) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (url) {
                        ...
                        [self convertMovSourceURL:url exportPath:kCameraVideoPath completion:^(BOOL isSuccess, NSURL *exportUrl) {
                            if (!isSuccess) {
                                [g_App showAlert:@"视频转换失败"];
                                return ;
                            }
                            //                        NSString *dataPath = kCameraVideoPath;
                            //                        [data writeToFile:dataPath atomically:YES];
                            // 获取视频时长
                            AVURLAsset *urlAsset = [AVURLAsset URLAssetWithURL:exportUrl options:nil];
                            NSInteger second = 0;
                            second = (NSInteger)urlAsset.duration.value / urlAsset.duration.timescale;
                            
                            WH_JXMediaObject* p = [[WH_JXMediaObject alloc] init];
                            p.userId = MY_USER_ID;
                            p.fileName = exportUrl.path;
...
-(void)convertMovSourceURL:(NSURL *)path exportPath:(NSString *)exportPath completion:(void(^)(BOOL isSuccess,NSURL *exportUrl))completion
{
    ...
    [exportSession exportAsynchronouslyWithCompletionHandler:^{
        dispatch_async(dispatch_get_main_queue(), ^{
            [MBProgressHUD hideHUDForView:g_window animated:YES];
        });
