...
@implementation WH_JXUserBaseObj
...
-(void)loadFromObject:(WH_JXUserBaseObj*)user{
    ...
    self.isGroupSignIn = user.isGroupSignIn;
...
-(void)userFromDataset:(WH_JXUserBaseObj*)obj rs:(FMResultSet*)rs{
    ...
    obj.isGroupSignIn = [rs objectForColumnName:KUSER_isGroupSignIn];
...
-(void)userFromDictionary:(WH_JXUserBaseObj*)obj dict:(NSDictionary*)aDic
{
    if([aDic count]<=0)
        return;
    ...
    obj.isGroupSignIn = [aDic objectForKey:KUSER_isGroupSignIn];
...
-(NSDictionary*)toDictionary
{
    ...
    if (self.isGroupSignIn) {
        [dic setObject:self.isGroupSignIn forKey:KUSER_isGroupSignIn];
    }
...
-(void)notifyDelFriend{
    [g_notify postNotificationName:kDeleteUser_WHNotifaction object:self userInfo:nil];
}

-(void)notifyNewFriend{
    [g_notify postNotificationName:kXMPPNewFriend_WHNotifaction object:self userInfo:nil];
}

- (void)notifyDelRoom {
    [g_notify postNotificationName:kDeleteRoom_WHNotifaction object:self userInfo:nil];
}
