...
-(instancetype)init{
    ...
    if(self){
        ...
        _navigationView.backgroundColor = g_factory.globalBgColor;
...
-(void)createHeaderView{
    if (self.wh_tableHeader) {
        [self.wh_tableHeader removeFromSuperview];
        self.wh_tableHeader = nil;
    }
    self.wh_tableHeader = [[UIView alloc]initWithFrame:CGRectMake(0, 0, JX_SCREEN_WIDTH, JX_SCREEN_TOP)];
...
//    }
    ...
    [self.wh_tableHeader addSubview:iv];
...
//    [self.wh_tableHeader addSubview:btn];
    ...
    [self.navigationView addSubview:self.wh_tableHeader];
}
- (void)actionQuit {
    UIViewController *vc = _subViews.lastObject;
    [self WH_dismiss_WHViewController:vc animated:YES];
}
...
- (void)screenEdgePanGestureRecognizer:(UIViewController *)viewController{
    ...
    if ([viewController isKindOfClass:[WH_admob_WHViewController class]]) {
        WH_admob_WHViewController *vc = (WH_admob_WHViewController *)viewController;
        [vc.wh_tableBody.panGestureRecognizer requireGestureRecognizerToFail:screenPan];
    }else if([viewController isKindOfClass:[WH_JXTableViewController class]]) {
        WH_JXTableViewController *vc = (WH_JXTableViewController *)viewController;
    ...
    if (screenPan.state == UIGestureRecognizerStateEnded) {
        ...
        if (p.x > JX_SCREEN_WIDTH/2 || velocity.x > 1000) {
            [UIView animateWithDuration:0.3 animations:^{
                ...
                if([vc isKindOfClass:[WH_JXTableViewController class]]) {
                    WH_JXTableViewController *tableVC = (WH_JXTableViewController *)vc;
                    [tableVC actionQuit];
                    [tableVC.footer removeFromSuperview];
                    [tableVC.header removeFromSuperview];
                    tableVC.header = nil;
                    tableVC.footer = nil;
                }else if([vc isKindOfClass:[WH_admob_WHViewController class]]){
                    WH_admob_WHViewController *admobVC = (WH_admob_WHViewController *)vc;
                    [admobVC actionQuit];
                }
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated{
    ...
    if ([viewController isKindOfClass:[WH_JXPayPassword_WHVC class]]) {
        
        //先判断是否绑定了手机号
        
//        if (g_myself.telephone == nil) {
//            MiXin_PSRegisterBase_MiXinVC *vc = [[MiXin_PSRegisterBase_MiXinVC alloc] init];
