...
@implementation WH_JXFriendObject

static WH_JXFriendObject *sharedUser;

+(WH_JXFriendObject*)sharedInstance{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedUser=[[WH_JXFriendObject alloc]init];
    });
    return sharedUser;
}
...
-(WH_JXFriendObject*)userFromDictionary:(NSDictionary*)aDic
{
    WH_JXFriendObject* user = [[WH_JXFriendObject alloc]init];
...
-(void)notifyNewRequest{
    //发送全局通知
    [g_notify postNotificationName:kXMPPNewRequest_WHNotifaction object:self userInfo:nil];
}
...
-(void)doDelUser{
    ...
    [WH_JXUserObject deleteUserAndMsg:self.userId];
}
...
-(NSString*)doMsgForNewUser{
    ...
    if([self.isMySend boolValue]){
        ...
        s = Localized(@"WaHu_JXFriendObject_ChatNow");
        p.isMySend    = YES;
    }else{
    ...
    if ([self.type intValue] == XMPP_TYPE_FRIEND) {
        ...
        s = Localized(@"WaHu_JXFriendObject_AddedFriend");
    }
    p.content      = s;
...
-(void)onSendRequest{
    ...
    WH_JXUserObject* user = [[WH_JXUserObject sharedUserInstance] getUserById:self.userId];
    if(user)
...
-(void)loadFromMessageObj:(WH_JXMessageObject*)msg{
    self.type = msg.type;
    ...
    if ([msg.type integerValue] == XMPP_TYPE_SEVERDEL) {
        ...
        self.userNickname = [self WH_getUserNameWithUserId:self.userId];
    }
    ...
    if ([msg.type integerValue] == XMPP_TYPE_SEVERBLACK || [msg.type integerValue] == XMPP_TYPE_SEVERNOBLACK || [msg.type integerValue] == XMPP_TYPE_SEVERDELFRIEND) {
        ...
        NSDictionary *resultObject = [msg.objectId mj_JSONObject];
    ...
    WH_JXFriendObject *user = [self selectUser:self.userId];
...
-(NSString*)getLastContent{
    ...
    if([self.isMySend boolValue]){
        switch (n) {
            case XMPP_TYPE_SAYHELLO:
                s= Localized(@"WaHu_JXFriendObject_WaitPass");
                break;
            case XMPP_TYPE_PASS:
            ...
            case XMPP_TYPE_SEVERBLACK:
            case XMPP_TYPE_BLACK:
                s = [NSString stringWithFormat:@"%@ %@",Localized(@"WaHu_JXFriendObject_AddedBlackList"),self.userNickname];
                break;
            case XMPP_TYPE_SEVERNOBLACK:
            case XMPP_TYPE_NOBLACK:
                s = [NSString stringWithFormat:@"%@ %@",MY_USER_NAME,Localized(@"WaHu_JXFriendObject_NoBlack")];
                break;
            case XMPP_TYPE_FRIEND:
                s = Localized(@"WaHu_JXFriendObject_AddedFriend");
                break;
        ...
        switch (n) {
            ...
            case XMPP_TYPE_SEVERDELFRIEND:
            case XMPP_TYPE_DELALL:
                s = [NSString stringWithFormat:@"%@ %@",self.userNickname,Localized(@"WaHu_JXFriendObject_Deleted")];
                break;
            case XMPP_TYPE_SEVERDEL:
                s = self.content;
                break;
            ...
            case XMPP_TYPE_SEVERBLACK:
            case XMPP_TYPE_BLACK:
                s = [NSString stringWithFormat:@"%@ %@",self.userNickname,Localized(@"WaHu_JXFriendObject_PulledBlack")];
                break;
            case XMPP_TYPE_SEVERNOBLACK:
            case XMPP_TYPE_NOBLACK:
                s = [NSString stringWithFormat:@"%@ %@",self.userNickname,Localized(@"WaHu_JXFriendObject_NoBlack")];
                break;
            case XMPP_TYPE_FRIEND:
                s = Localized(@"WaHu_JXFriendObject_BeAddFriend");
                break;
            case XMPP_TYPE_CONTACTFRIEND:
                s = Localized(@"JX_FriendsViaMobileContacts");
                break;
...
// 查找user
- (WH_JXFriendObject *)selectUser:(NSString *)userId {
    FMDatabase* db = [[JXXMPP sharedInstance] openUserDb:MY_USER_ID];
...
-(WH_JXFriendObject*)getFriendById:(NSString*)userId {
    FMDatabase* db = [[JXXMPP sharedInstance] openUserDb:MY_USER_ID];
