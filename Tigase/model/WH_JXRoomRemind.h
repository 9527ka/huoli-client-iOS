...
#define kRoomRemind_DelMember 904 //删除群成员
#define kRoomRemind_NewNotice 905   //群公告提醒
#define kRoomRemind_DisableSay 906 // 禁言/取消禁言  个人禁言
...
#define kRoomRemind_GroupSignIn 934 //开启、关闭群签到功能

#define kRoomRemind_EditUserInfo 2000 //编辑个人信息

@interface WH_JXRoomRemind : NSObject{ //房间控制消息，用于发通知，并不保存到数据库
    NSString* _tableName;
}
...
+(WH_JXRoomRemind*)sharedInstance;
+(void)createAndNotifyNewObj:(NSString*)objectId toUserId:(NSString*)toUserId type:(int)type;


//将对象转换为字典
-(NSDictionary*)toDictionary;
-(void)fromDataset:(WH_JXRoomRemind*)obj rs:(FMResultSet*)rs;
-(void)fromDictionary:(WH_JXRoomRemind*)obj dict:(NSDictionary*)aDic;
