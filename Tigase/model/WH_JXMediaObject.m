...
@implementation WH_JXMediaObject
...
static WH_JXMediaObject *shared;

+(WH_JXMediaObject*)sharedInstance{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        shared=[[WH_JXMediaObject alloc] init];
    });
    return shared;
}
...
-(void)fromDataset:(WH_JXMediaObject*)obj rs:(FMResultSet*)rs{
...
-(void)fromDictionary:(WH_JXMediaObject*)obj dict:(NSDictionary*)aDic
{
...
-(NSMutableArray*)fetch
{
    ...
    while ([rs next]) {
        WH_JXMediaObject *user=[[WH_JXMediaObject alloc] init];
