...
- (void)baiduTranslation:(WH_JXMessageObject *)msg{
    
    NSMutableDictionary *p = [NSMutableDictionary new];
    
    [p setValue:msg.content forKey:@"q"];
    [p setValue:@"auto" forKey:@"from"];
    [p setValue:@"en" forKey:@"to"];
    ...
    NSString *md5Str = [g_server WH_getMD5StringWithStr:[NSString stringWithFormat:@"%@%@%dfpaFBj3q6DivDokLCXfe",appid,msg.content,x]];
    [p setValue:md5Str forKey:@"sign"];
    
    __block NSString *newContext = @"";
...
//        NSString *string = [[NSString alloc] initWithData:responseObject encoding:NSUTF8StringEncoding];
        ...
        NSDictionary *trans_resultDic = dic[@"trans_result"];
        if([trans_resultDic isKindOfClass:NSDictionary.class]){
            newContext = trans_resultDic[@"dst"];
        }else if ([trans_resultDic isKindOfClass:NSArray.class]){
            NSArray *arr = (NSArray *)trans_resultDic;
            if(arr.count){
                trans_resultDic = arr.firstObject;
                newContext = trans_resultDic[@"dst"];
            }
        }

        if(newContext && newContext.length){
            msg.content = newContext;
        }
