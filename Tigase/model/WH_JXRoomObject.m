...
@implementation WH_JXRoomObject
...
-(void)joinRoom:(bool)isNew{
    ...
    if(g_server.lastOfflineTime>0&&!isNew){
        NSArray *arr = [[WH_JXMessageObject sharedInstance] fetchMessageListWithUser:self.roomJid byAllNum:0 pageCount:20 startTime:[NSDate dateWithTimeIntervalSince1970:0]];
        // 最后一条消息
...
//    //NSLog(@"config : %@", configForm);
    ...
    for (NSXMLElement *field in fields) {
        ...
        if ([var isEqualToString:@"muc#roomconfig_persistentroom"]) {
            ...
            [field WH_addChild:[NSXMLElement elementWithName:@"value" stringValue:@"1"]];
        }
    }
...
-(void)configNewRoom{
    ...
    [p WH_addChild:[NSXMLElement elementWithName:@"value" stringValue:@"1"]];
    [x WH_addChild:p];
    
    p = [NSXMLElement elementWithName:@"field" ];
    [p addAttributeWithName:@"var" stringValue:@"muc#roomconfig_maxusers"];//最大用户
    [p WH_addChild:[NSXMLElement elementWithName:@"value" stringValue:@"1000"]];
    [x WH_addChild:p];
    ...
    [p WH_addChild:[NSXMLElement elementWithName:@"value" stringValue:self.roomTitle]];
    [x WH_addChild:p];
    
    p = [NSXMLElement elementWithName:@"field" ];
    [p addAttributeWithName:@"var" stringValue:@"muc#roomconfig_enablelogging"];//允许登录对话
    [p WH_addChild:[NSXMLElement elementWithName:@"value" stringValue:@"0"]];
    [x WH_addChild:p];
    */
...
-(void)removeUser:(memberData*)user{
    ...
    [sub WH_addChild:[NSXMLElement elementWithName:@"reason" stringValue:@"Avaunt, you cullion!"]];
    [query WH_addChild:sub];
