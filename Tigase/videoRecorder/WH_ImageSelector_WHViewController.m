//
//  WH_ImageSelector_WHViewController.m
//  Tigase_imChatT
//
...
@interface WH_ImageSelector_WHViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>{
    
    CGRect _initFrame;
}

@property (nonatomic, strong) WH_JXCollectionView * collectionView;
@property (nonatomic, assign) NSInteger selectIndex;


@end

@implementation WH_ImageSelector_WHViewController
...
-(void)customView{
    ...
    _collectionView = [[WH_JXCollectionView alloc] initWithFrame:CGRectMake(0, 0, self.wh_tableBody.frame.size.width, self.wh_tableBody.frame.size.height) collectionViewLayout:layout];
//    [layout release];
    ...
    [_collectionView registerClass:[WH_ImageSelector_WHCollectionCell class] forCellWithReuseIdentifier:imageSelectCellID];
    [self.wh_tableBody addSubview:_collectionView];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    WH_ImageSelector_WHCollectionCell * cell = [collectionView dequeueReusableCellWithReuseIdentifier:imageSelectCellID forIndexPath:indexPath];
...
-(void)didImageView:(UIView*)sender{
    ...
    WH_ImageSelector_WHCollectionCell * cell = (WH_ImageSelector_WHCollectionCell*)[_collectionView cellForItemAtIndexPath:indexPath];
    _initFrame = [cell.contentView convertRect:cell.wh_imageView.frame toView:self.view];
    UIImage * image = [cell.wh_imageView image];
...
-(void)changeCell:(long)index selected:(BOOL)value{
    ...
    if(value)
        cell.wh_selectView.image = [UIImage imageNamed:@"selected_true"];
    else
        cell.wh_selectView.image = [UIImage imageNamed:@"selected_fause"];
