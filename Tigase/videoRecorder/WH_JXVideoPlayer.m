...
@implementation WH_JXVideoPlayer
...
//        CGRect frame = THE_DEVICE_HAVE_HEAD ? CGRectMake(0, JX_SCREEN_TOP, JX_SCREEN_WIDTH, JX_SCREEN_HEIGHT-JX_SCREEN_TOP-JX_SCREEN_BOTTOM) : _firstBaseView.bounds;
        ...
        _videoFirst.contentMode = UIViewContentModeScaleAspectFit;
        [_firstBaseView addSubview:_videoFirst];
        [FileInfo getFullFirstImageFromVideo:_videoFile imageView:_videoFirst];
        
        _wait = [[WH_JXWaitView alloc] initWithParent:_firstBaseView];
        [_wait WH_start];
...
//        [g_notify addObserver:self selector:@selector(playerPause:) name:kAllVideoPlayerPauseNotifaction object:nil];//开始录音
        ...
        [g_notify addObserver:self selector:@selector(exitVideoPlayer) name:@"CancleVideoPlay_Notification" object:nil];
    }
    return self;
}
...
- (void)longPress:(UILongPressGestureRecognizer *)gestureRecognizer {
    WH_JXActionSheet_WHVC *actionVC = [[WH_JXActionSheet_WHVC alloc] initWithImages:@[] names:@[Localized(@"JX_SaveVideo")]];
...
//显示播放器
- (void)start{
    _player = [[WH_JXVideoPlayer_WHVC alloc] init];
    _player.isVideo = self.isVideo;
    _player.wh_pauseButton = _pauseBtn;
    _player.delegate = self;
    _player.wh_parent = _parent;
    _player.wh_timeCur = _timeLab;
    _player.wh_timeEnd = _timeEnd;
    _player.wh_movieTimeControl = _movieTimeControl;
    ...
    if (_isPreview) {  // 预览界面处理
        _timeEnd.hidden = YES;
        _player.wh_movieTimeControl.hidden = YES;
        _pauseBtn.hidden = YES;
        _exitBtn.hidden = YES;
        _timeLab.hidden = YES;
        _exitBtn.hidden = YES;
        _disBtn = [self WH_create_WHButtonWithFrame:CGRectMake(20, JX_SCREEN_HEIGHT-20-80, 80, 80) image:@"video_return" action:@selector(exitVideoPlayer)];
//        [_parent addSubview:_disBtn];
//        [_parent bringSubviewToFront:_disBtn];
        _sendBtn = [self WH_create_WHButtonWithFrame:CGRectMake(JX_SCREEN_WIDTH-20-80, JX_SCREEN_HEIGHT-20-80, 80, 80) image:@"video_gou" action:@selector(didButtonWithSendVideo)];
//        [_parent addSubview:_sendBtn];
//        [_parent bringSubviewToFront:_sendBtn];
    ...
    //修复视频未完全加载时播放点击无法弹出顶部和底部工具条,以至无法退出播放页
    [self adjust];
//    if(_player.isOpened)
//        [self doVideoOpen];
}
...
-(void)doPlayEnd{
    ...
    self.isPlaying = NO;
    if(self.delegate != nil && [self.delegate respondsToSelector:self.WH_didVideoPlayEnd])
        [self.delegate performSelectorOnMainThread:self.WH_didVideoPlayEnd withObject:self waitUntilDone:NO];
}
...
- (UIButton *)WH_create_WHButtonWithFrame:(CGRect)frame image:(NSString *)image action:(SEL)action {
