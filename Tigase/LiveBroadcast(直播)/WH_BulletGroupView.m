...
@implementation WH_BulletGroupView
...
-(void)loadButtetView
{
    ...
    for (int i = j; i > 0; i--)
    {
        ...
        if (comment)
        {
            ...
            [self WH_createWH_BulletView:comment trajectory:trajectory];
        }
        else
        {
            //没有评论时
...
- (void)WH_createWH_BulletView:(NSString *)comment trajectory:(int)trajectory
{
    if (self.isStopped)return;
    
    //创建WH_BulletView，如果有移除来的，那么重用；
    WH_BulletView *view;
    ...
    {
        view = [[WH_BulletView alloc] initWithCommentDic:_bulletDic];
    }
    view.wh_trajectory = trajectory;
    [_bulletArray addObject:view];
    
    __unsafe_unretained WH_BulletView *weakWH_BulletView = view;
    __unsafe_unretained WH_BulletGroupView *weakSelf = self;
    
    view.wh_moveBlock = ^(CommentMoveStatus status)
    {
        ...
        switch (status) {
            ...
            case Enter: {
                ...
                if (comment)
                {
                    [weakSelf WH_createWH_BulletView:comment trajectory:trajectory];
                } else
                {
                    //此时还有没完全走出去的,那么没走出去一条，就重新看一遍有没有新来的，如果有就加进去一条
                    [weakSelf WH_getMoreComment];
                    NSString *moreComment = [weakSelf nextComment];
                    if (moreComment)
                    {
                        [weakSelf WH_createWH_BulletView:moreComment trajectory:trajectory];
                    }else
                    {
                        //可用的没了，要保存下顺序数组
            ...
            case MoveOut:
            {
                ...
                if ([weakSelf.bulletArray containsObject:weakWH_BulletView])
                {
    ...
    //弹幕生成后，加到视图上
    view.frame = CGRectMake(CGRectGetWidth(self.frame)+CGRectGetWidth(view.bounds)-20, 34 * view.wh_trajectory, CGRectGetWidth(view.bounds), CGRectGetHeight(view.bounds));
...
//这个字典中设置弹幕的字体和颜色，具体哪些键，请参看WH_BulletView.h中的键定义。
...
-(void)startBullet
{
    if (self.tmpLoadDataArray.count == 0)[self WH_getMoreComment];
    self.isStopped = NO;
    [self loadButtetView];
}
