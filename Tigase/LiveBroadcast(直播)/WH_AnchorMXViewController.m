...
@interface WH_AnchorMXViewController ()<LFLiveSessionDelegate,UIAlertViewDelegate>{
...
@implementation WH_AnchorMXViewController
...
- (void)liveSession:(LFLiveSession *)session liveStateDidChange:(LFLiveState)state {
    ...
    if (state == LFLiveError) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            _session = nil;
            [g_server liveRoomStatus:0 roomId:self.wh_liveRoomId toView:nil];
            [super quitLiveRoom];
        });
    }
}
...
-(void)startLiveButtonAction:(UIButton *)button{
    ...
    if(_startLiveButton.selected){
        ...
        stream.url = self.wh_liveUrl;
        _session.showDebugInfo = YES;
        [_session startLive:stream];
        [g_server liveRoomStatus:1 roomId:self.wh_liveRoomId toView:self];
    }else{
        [_startLiveButton setTitle:Localized(@"JXLiveVC_StartLive") forState:UIControlStateNormal];
//        [_session stopLive];
