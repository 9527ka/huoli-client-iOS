...
@interface WH_JXProgress_WHVC ()

@end

@implementation WH_JXProgress_WHVC
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.title = Localized(@"WaHu_JXProgress_WaHuVC_SnyFriend");
...
//    [_comBtn setTitle:Localized(@"WaHu_JXProgress_WaHuVC_SnyNow") forState:UIControlStateNormal];
//    [_comBtn setBackgroundImage:[UIImage imageNamed:@"feaBtn_backImg_sel"] forState:UIControlStateNormal];
//    _comBtn.titleLabel.font = sysFontWithSize(15);
...
-(void)createProgressView{
    ...
    [self.wh_tableBody addSubview:centerView];
//        [centerView release];
...
//        [_wait start:Localized(@"WaHu_JXProgress_WaHuVC_SnyFriend")];
//    });
    
//    _titleLabel.text = @"正在同步好友";
    
    // 删除本地数据库好友 重新插入
//    [[WH_JXUserObject sharedUserInstance] deleteAllUser];
...
//        [user release];
    ...
    // 删除服务器上已经删除的
//    NSArray *arr = [g_myself WH_fetchAllFriendsOrNotFromLocal];
//    for (NSInteger i = 0; i < arr.count; i ++) {
//        WH_JXUserObject *locUser = arr[i];
//        BOOL flag = NO;
//        for (NSInteger j = 0; j < array1.count; j ++) {
//            NSDictionary * dict = array1[j];
//            WH_JXUserObject * serverUser = [[WH_JXUserObject alloc]init];
//            //数据转为一个好友对象
//            [serverUser WH_getDataFromDictSmall:dict];
...
//    _dbCountLabel.text = _dbCountLabel.text = [NSString stringWithFormat:@"%@%ld",Localized(@"WaHu_JXProgress_WaHuVC_LFriendCount"),[array1 count]];
//    _sysCountLabel.text = [NSString stringWithFormat:@"%@%ld",Localized(@"WaHu_JXProgress_WaHuVC_SFriendCount"),[array1 count]];
//    [_comBtn setTitle:Localized(@"JX_Finish") forState:UIControlStateNormal];
//    _comBtn.tag = 2;
