...
#define INSET_INT 12    //   左右间隔

@implementation WH_JX_SelectMenuView
...
- (instancetype)initWithTitle:(NSArray *)titleArr image:(NSArray *)images cellHeight:(int)height {
    ...
    if (self) {
        ...
        CGSize size = [currentStr boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:sysFontWithSize(16)} context:nil].size;
...
//        self.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:.3];
        ...
        CGPoint point;
        CGFloat width = 145.0f;
        if (images.count > 0) {  // 有图标
//            point = CGPointMake(JX_SCREEN_WIDTH - (size.width + INSET_INT*4) - 16, JX_SCREEN_TOP);
//            bigView.frame = CGRectMake(point.x, point.y+3 , size.width+INSET_INT*4, titleArr.count *height+topHeight);
            point = CGPointMake(JX_SCREEN_WIDTH - width - 10, JX_SCREEN_TOP);
            bigView.frame = CGRectMake(point.x, point.y , width, titleArr.count *height+topHeight);
        } else {  // 无图标
