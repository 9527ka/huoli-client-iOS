...
@interface WH_JXSetLabel_WHVC ()<UITextFieldDelegate,UITextViewDelegate,UIScrollViewDelegate>
...
@implementation WH_JXSetLabel_WHVC
...
- (void)viewDidLoad{
    ...
    if (_array.count <= 0) {
        _array = [[WH_JXLabelObject sharedInstance] fetchLabelsWithUserId:self.user.userId];
    }
    if (_allArray.count <= 0) {
        _allArray = [[WH_JXLabelObject sharedInstance] fetchAllLabelsFromLocal];
    }
...
//    p.frame = THE_DEVICE_HAVE_HEAD ? CGRectMake(JX_SCREEN_WIDTH -45, 20+10+23, 35, 25) : CGRectMake(JX_SCREEN_WIDTH -90, 20+10, 80, 25);
    ...
    [self.wh_tableBody addSubview:_scrollView];
    
    _allScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_scrollView.frame), JX_SCREEN_WIDTH, self.wh_tableBody.frame.size.height - CGRectGetMaxY(_scrollView.frame))];
    _allScrollView.backgroundColor = [UIColor clearColor];
    [self.wh_tableBody addSubview:_allScrollView];
...
- (void)createLabels {
   ...
   [_scrollView.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
    ...
    for (NSInteger i = 0; i < _array.count; i ++) {
        ...
        if (i != _array.count - 1) {
            WH_JXLabelObject *lastLabelObj = _array[i + 1];
...
- (void) createAllLabels {
    ...
    for (NSInteger i = 0; i < _allArray.count; i ++) {
        WH_JXLabelObject *labelObj = _allArray[i];
        
        BOOL flag = NO;
        for (WH_JXLabelObject *obj in _array) {
        ...
        if (i != _allArray.count - 1) {
            WH_JXLabelObject *lastLabelObj = _allArray[i + 1];
...
- (void)allLabelAction:(UIButton *)labelBtn {
    ...
    WH_JXLabelObject *allObj = _allArray[labelBtn.tag];
    WH_JXLabelObject *obj = nil;
    // 查找已选标签中是否有此标签
    for (WH_JXLabelObject *labelObj in _array) {
