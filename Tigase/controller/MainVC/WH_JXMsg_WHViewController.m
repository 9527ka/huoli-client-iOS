...
//    else if ([dict.message.type intValue] == kWCMessageTypeRedPacketReturn){//the merchant paid the seller
//        lastContet = @"[红包退回]";
//    }
...
-(void)WH_on_WHHeadImage:(UIView*)sender{
    ...
    if([p.user.roomFlag boolValue] || p.user.roomId.length > 0) {
        ...
        if (s.length > 0) {
            ...
            vc.wh_room = [[appDelegate.mainVc getGroupVC] getRoomDataFromRoomId:p.user.roomId];
            vc.pData = p;
...
-(void)showNewCount{//显示IM数量
    ...
    [g_mainVC.tb wh_setBadge:1 title:[NSString stringWithFormat:@"%d",self.wh_msgTotal]];
}
