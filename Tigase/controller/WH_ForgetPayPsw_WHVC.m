...
- (void)viewDidLoad {
    ...
    self.title = Localized(@"New_Forgot_payment_password");
...
- (void)commonInit{
    _items = @[
               @[@{
                     @"title":Localized(@"New_password_check"),
                     @"content":@"",
                     @"type":@(WHSettingCellTypeTitleWithContent),
                     },
                 @{
                     @"title":Localized(@"New_password"),
                     @"content":@"",
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        [cell.button setTitle:Localized(@"New_next_step") forState:UIControlStateNormal];
