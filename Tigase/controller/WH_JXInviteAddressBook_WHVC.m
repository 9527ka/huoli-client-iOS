...
@interface WH_JXInviteAddressBook_WHVC ()<WH_JXAddressBook_WHCellDelegate, QCheckBoxDelegate,MFMessageComposeViewControllerDelegate>
...
@implementation WH_JXInviteAddressBook_WHVC
...
- (void)viewDidLoad{
    ...
    self.title = Localized(@"New_Invite_friends");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGRect frame = cell.headImage.frame;
    frame.size.width = .0f;
    cell.headImage.frame = frame;
    [cell.verticalLine mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.left.offset(-g_factory.cardBorderWithd);
        make.top.bottom.offset(0);
        make.width.offset(g_factory.cardBorderWithd);
    }];
