...
- (void)commonInit{
    ...
    if ([g_config.wechatLoginStatus integerValue]==1 || [g_config.qqLoginStatus integerValue]==1) {
        [mainArr addObject:@[@{
                                 @"title":Localized(@"JX_EquipmentLock"),
                                 @"content":@"",
    ...
    [mainArr addObject:@[@{
        ...
        @"title":![g_myself.isPayPassword boolValue]?@"设置支付密码":@"修改支付密码",
                         ...
                         @{
                             ...
                             @"title":Localized(@"New_Forgot_payment_password"),
                             @"content":@"",
                             @"type":@(WHSettingCellTypeTitleWithContent),
                             },
                         ]];
    ...
    if ([g_config.isQestionOpen integerValue] == 1) {
        [mainArr addObject:@[@{
                                 ...
                                 @"title":Localized(@"New_security_question"),
                                 @"content":Localized(@"New_Not_set"),
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 2 && indexPath.row == 0) {//密保问题
        if (g_myself.questions.count > 0) {
            ...
            cell.contentLabel.text = Localized(@"New_Not_set");
        }
...
-(WH_JXImageView*)WH_createMiXinButton:(NSString*)title drawTop:(BOOL)drawTop drawBottom:(BOOL)drawBottom must:(BOOL)must click:(SEL)click superView:(UIView *)superView{
    ...
    if(click){
        UIImageView* iv;
        iv = [[UIImageView alloc] initWithFrame:CGRectMake(JX_SCREEN_WIDTH - 2*g_factory.globelEdgeInset - 21, (55 - 12)/2, 14, 14)];
