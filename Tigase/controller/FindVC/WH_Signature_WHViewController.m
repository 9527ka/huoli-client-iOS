...
-(void) WH_didServerResult_WHSucces:(WH_JXConnection*)aDownload dict:(NSDictionary*)dict array:(NSArray*)array1{
    ...
    if([aDownload.action isEqualToString:wh_act_UserSign] ){
        ...
        if ([[dict objectForKey:@"seriesSignCount"] integerValue] > 0) {
            self.seriesDayLbl.text = [NSString stringWithFormat: @"%@%@%@",@"You have continuously signed in",[dict objectForKey:@"seriesSignCount"],@"day"];
        }
        [self day7SignStatus:dict];
...
- (int) WH_didServerResult_WHFailed:(WH_JXConnection*)aDownload dict:(NSDictionary*)dict{
    [_wait hide];
    NSString *errorMsg = [dict objectForKey:@"resultMsg"]?:Localized(@"New_error_please_try_again");
...
-(int) WH_didServerConnect_WHError:(WH_JXConnection*)aDownload error:(NSError *)error{//error为空时，代表超时
    [_wait hide];
    [g_App showAlert:Localized(@"New_error_please_try_again")];
    return WH_hide_error;
}
...
-(void) signBtnDisable{
    ...
    self.tip1Lbl.text = [NSString stringWithFormat:@"Congratulations on receiving%@Red envelopes" ,appName];
    [self.signBtn setTitle:@"Signed in today" forState:UIControlStateNormal];
