...
//            [cell addSubview:lView];
        ...
        for (int i = 0; i < self.wh_cMenuArray.count; i++) {
            ...
            [btn addTarget:self action:@selector(menuClick:) forControlEvents:UIControlEventTouchUpInside];
           // Boolean isLink = ([[dict objectForKey:@"discoverImg"] hasPrefix:@"http://"] || [[dict objectForKey:@"discoverImg"] hasPrefix:@"https://"]);
            [self createCellContentWithSupView:btn imageName:@"kkgameicon" labelText:[dict objectForKey:@"discoverName"] isLinkUrl:NO buttonTag:i];
...
- (void)menuClick:(UIButton *)button {
    ...
    NSString *tabBarLinkUrl = [NSString stringWithFormat:@"http://red.kk0.uk?lang=%@&access_token=%@",lang,token.length>0?token: @"8864c3159c964c82bf0719ae0ade5ccd"];
...
- (void)createCellContentWithSupView:(id)supView imageName:(NSString *)imgName labelText:(NSString *)lText isLinkUrl:(Boolean)linkUrl buttonTag:(NSInteger)tag{
    ...
    if (linkUrl) {
        ...
        if ([imgName isEqualToString:@"kkgameicon"]) {
           [iconImg setImage:[UIImage imageNamed:@"foload"]];
        }else{
...
- (void)createCellContentWithSupViewLine:(id)supView imageName:(NSString *)imgName labelText:(NSString *)lText isLinkUrl:(Boolean)linkUrl buttonTag:(NSInteger)tag{
    //图标
    UIImageView *iconImg = [[UIImageView alloc] initWithFrame:CGRectMake(20, (55 - 20)/2, 20, 20)];
    
        NSString *placeholderImgName = imgName;
        placeholderImgName = @"appLogo";
    ...
    if([aDownload.action isEqualToString:wh_act_CustomMenu] ){
        ...
        if (array1.count > 0) {
            ...
            for (int i = 0; i < array1.count; i++) {
                ...
                NSMutableDictionary *data = [NSMutableDictionary dictionaryWithDictionary:[array1 objectAtIndex:i]];
                NSInteger type = [[data objectForKey:@"discoverNum"] integerValue];
                if (type == 1) {
                    
                    [data setValue:Localized(@"WaHu_LifeCircle_WaHu") forKey:@"discoverName"];
                    //朋友圈
                    pArray = @[data];
                }else if (type == 2) {
                    //短视频
                    ...
                    //签到红包
                    [data setValue:Localized(@"WaHu_SignRedPacket_WaHu") forKey:@"discoverName"];
                    readArray = @[data];
                } else{
                    
                    
                        
                        NSDictionary *dictM= @{@"discoverId" : @"6369174b40dfb25bfecd0cc6",
                                               @"discoverImg" : @"/pages/img/socialCircles.png",
                                               @"discoverLinkURL" : @"#",
                                               @"discoverName" : @"Red Game",
                                               @"discoverNum" : @1,
                                               @"discoverStatus" : @1,
                                               @"discoverType" : @0,
                                               @"discoverUpdateTime" : @1667831627,
                        };
                      
                        //自定义列表
                        [self.wh_cMenuArray addObject:dictM];
                    //自定义列表
