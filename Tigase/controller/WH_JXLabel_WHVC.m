...
//    return cell;
    ...
    if([label.groupName isEqualToString:Localized(@"New_family")]){
        
        cell.titleLabel.text = [NSString stringWithFormat:@"%@ (%lu)",Localized(@"New_family"), (unsigned long)userIds.count];
    }else if ([label.groupName isEqualToString:Localized(@"New_colleague")]){
        cell.titleLabel.text = [NSString stringWithFormat:@"%@ (%lu)",Localized(@"New_colleague"), (unsigned long)userIds.count];
    }
