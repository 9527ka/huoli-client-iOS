...
@interface WH_JXSelectLabels_WHVC ()
...
@implementation WH_JXSelectLabels_WHVC
...
- (void)viewDidLoad{
    ...
    [allSelect addTarget:self action:@selector(WH_confirmBtnAction:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)didSelectedCheckBox:(QCheckBox *)checkbox checked:(BOOL)checked{
    WH_JXLabelObject *labelObj = _array[checkbox.tag];
    ...
    if(checked){
        ...
        for (NSInteger i = 0; i < _selLabels.count; i ++) {
            WH_JXLabelObject *selLabel = _selLabels[i];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    BOOL flag = NO;
    for (NSInteger i = 0; i < _selLabels.count; i ++) {
        WH_JXLabelObject *selLabelObj = _selLabels[i];
