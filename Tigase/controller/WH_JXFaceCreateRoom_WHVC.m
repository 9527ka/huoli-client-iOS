...
#import "WH_HWTFCodeView.h"
#import "WH_JXChat_WHViewController.h"
#import "WH_JXRoomPool.h"
#import "WH_JXUserObject.h"

#define HEIGHT 50
@interface WH_JXFaceCreateRoom_WHVC ()<WH_HWTFCodeViewDelegate>

@property (nonatomic, strong) UILabel *tip;
@property (nonatomic, strong) UILabel *joinTip;
@property (nonatomic, strong) WH_HWTFCodeView *codeView;
...
@implementation WH_JXFaceCreateRoom_WHVC
...
- (void)viewDidLoad{
    ...
    title.didTouch = @selector(WH_actionTitle:);
    title.wh_delegate = self;
    title.wh_changeAlpha = NO;
    ...
    WH_HWTFCodeView *code1View = [[WH_HWTFCodeView alloc] initWithCount:4 margin:10];
    ...
    _btn = [UIFactory WH_create_WHCommonButton:Localized(@"JX_AccessToGroup") target:self action:@selector(onJoin)];
...
-(void)msgRoomFaceNotif:(NSNotification *)notifacation {
    ...
    [g_server WH_roomLocationQueryWithIsQuery:1 password:self.password toView:self];
}
...
- (void)createUsers {
    ...
    for (NSInteger i = 0; i < _array.count; i ++) {
        ...
        [head headRadiusWithAngle:w * 0.5];
        [g_server WH_getHeadImageLargeWithUserId:dict[@"userId"] userName:dict[@"nickname"] imageView:head];
...
- (void)codeView:(WH_HWTFCodeView *)codeView inputFnish:(NSString *)text {
    ...
    self.password = text;
    [g_server WH_roomLocationQueryWithIsQuery:0 password:text toView:self];
    
}

- (void)onJoin {
    [g_server WH_roomLocationJoinWithJid:self.jid toView:self];
}

- (void)cancelAction {
 
    [g_server WH_roomLocationExitWithJid:self.jid toView:self];
    [self actionQuit];
...
-(void) WH_didServerResult_WHSucces:(WH_JXConnection*)aDownload dict:(NSDictionary*)dict array:(NSArray*)array1{
    [_wait stop];
    if( [aDownload.action isEqualToString:wh_act_RoomLocationQuery] ){
    ...
    if ([aDownload.action isEqualToString:wh_act_RoomLocationJoin]) {
...
-(void)onNewRoom:(WH_JXUserObject *)user createName:(NSString *)createName{
