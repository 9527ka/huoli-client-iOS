...
@implementation WH_NumLock_WHViewController
...
- (void)initNumLockKeyboard
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        WH_NumLockButton * numBu;
        if (i == 0)
        {
            numBu = [[WH_NumLockButton alloc] initWithNumber:i letters:@""];
            [numBu setFrame:CGRectMake((JX_SCREEN_WIDTH - 64) / 2, (__MainScreen_Height - 218)/2 + 228, 64, 64)];
        }
        else
        {
            numBu = [[WH_NumLockButton alloc] initWithNumber:i letters:[self lettersForNum:i]];
            CGFloat x = 0;
            if ((i - 1) % 3 == 0) {
