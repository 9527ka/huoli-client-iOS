...
@implementation WH_JXInput_WHVC
...
//        self.parent.alpha = 0.5;
        ...
        if(!inputHint)
            self.inputHint = Localized(@"WaHu_JXInput_WaHuVC_InputReply");
        if(!inputBtn)
            self.inputBtn = Localized(@"JX_Send");
        if (!_titleFont) {
            self.titleFont = sysBoldFontWithSize(15);
        }
        if (!_titleColor) {
...
//        [iv release];
        ...
        JXLabel* p;
        p = [self WH_createLabel:iv default:inputTitle];
...
//        }
        ...
        _value = [self WH_createMiXinTextField:iv default:inputText hint:inputHint];
...
-(JXLabel*)WH_createLabel:(UIView*)parent default:(NSString*)s{
