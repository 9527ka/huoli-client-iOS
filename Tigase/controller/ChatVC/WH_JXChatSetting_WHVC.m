...
//        [mbView addSubview:iv];
    ...
    UIView *zhView = [self createSubViewWithFrame:CGRectMake(g_factory.globelEdgeInset, hhh, w, HEIGHT)];
...
-(WH_JXImageView*)MiXin_createMiXinButton:(NSString*)title superView:(UIView *)view orginY:(CGFloat)orginY drawTop:(BOOL)drawTop drawBottom:(BOOL)drawBottom icon:(NSString*)icon click:(SEL)click {
    ...
    if(click){
        UIImageView* iv;
        iv = [[UIImageView alloc] initWithFrame:CGRectMake(CGRectGetWidth(view.frame) - 21, (HEIGHT-12)/2, 14, 14)];
