...
@interface WH_JXLocMap_WHVC ()<BMKMapViewDelegate,BMKLocationServiceDelegate>{
...
@implementation WH_JXLocMap_WHVC
...
- (void)customView{
    ...
    //复位
    _resetLoca = [[UIButton alloc]initWithFrame:CGRectMake(20,self.view.frame.size.height - JX_SCREEN_BOTTOM, 30, 30)];
...
//    [_jobDetailThree release];
    ...
    //放大缩小
    _scaleView = [[UIView alloc]initWithFrame:CGRectMake(JX_SCREEN_WIDTH - 100, self.view.frame.size.height - JX_SCREEN_BOTTOM , 90, 30)];
...
//        [_detailLabelOne release];
        ...
        [_headImageOne headRadiusWithAngle:_headImageOne.frame.size.width * 0.5];
...
//        [_detailLabelTwo release];
        ...
        [_headImageTwo headRadiusWithAngle:_headImageOne.frame.size.width * 0.5];
...
//        [_detailLabelThree release];
        ...
        [_headImageThree headRadiusWithAngle:_headImageOne.frame.size.width * 0.5];
...
-(void)setOneData:(int)centerIndex{
    ...
    //头像
    [g_server WH_getHeadImageSmallWIthUserId:[NSString stringWithFormat:@"%lld",[dataDict[@"userId"] longLongValue]] userName:dataDict[@"nickname"] imageView:_headImageOne];
    
    //昵称
    //    _nameLabelOne.text = dataDict[@"nickname"];
    
    //距离
    _distanceLabelOne.text = [NSString stringWithFormat:@"%.3fkm",[g_server WH_getLocationWithLatitude:[dataDict[@"loc"][@"lat"] doubleValue] longitude:[dataDict[@"loc"][@"lng"] doubleValue]]/1000];
...
//    _detailLabelTwo.text = twoData[@"loc"][@"lat"];
    //头像
    [g_server WH_getHeadImageSmallWIthUserId:[NSString stringWithFormat:@"%lld",[dataDict[@"userId"] longLongValue]] userName:dataDict[@"nickname"] imageView:_headImageTwo];
    
    //昵称
//    _nameLabelTwo.text = twoData[@"nickname"];
    
    //距离
    _distanceLabelTwo.text = [NSString stringWithFormat:@"%.3fkm",[g_server WH_getLocationWithLatitude:[dataDict[@"loc"][@"lat"] doubleValue] longitude:[dataDict[@"loc"][@"lng"] doubleValue]]/1000];
}
-(void)setThreeData:(int)centerIndex{
    ...
    //    _detailLabelTwo.text = twoData[@"loc"][@"lat"];
    //头像
    [g_server WH_getHeadImageSmallWIthUserId:[NSString stringWithFormat:@"%lld",[dataDict[@"userId"] longLongValue]] userName:dataDict[@"nickname"] imageView:_headImageThree];
    
    //昵称
    //    _nameLabelTwo.text = twoData[@"nickname"];
    
    //距离
    _distanceLabelThree.text = [NSString stringWithFormat:@"%.3fkm",[g_server WH_getLocationWithLatitude:[dataDict[@"loc"][@"lat"] doubleValue] longitude:[dataDict[@"loc"][@"lng"] doubleValue]]/1000];
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    if (scrollView.tag == 12) {
        ...
        //动画
        [_lastAnnotationView wh_cancelSelectAnimation];
        for ( int i = 0; i < [_allAnnotationView count]; i++) {
            WH_JXLocPerImage_WHVC * annoView = [_allAnnotationView objectAtIndex:i];
...
- (void)didUpdateBMKUserLocation:(BMKUserLocation *)userLocation
{
    ...
    if (self.locX == 0 && self.locY ==0) {
        ...
        g_server.latitude = self.locY;
        g_server.longitude = self.locX;
...

{
    ...
    WH_JXLocPerImage_WHVC *annotationView = [[WH_JXLocPerImage_WHVC alloc] initWithAnnotation:annotation reuseIdentifier:AnnotationViewID];
...
//    annotationView.canShowCallout = YES;//在点击大头针的时候会弹出那个黑框框
    ...
    [annotationView wh_setData:[_nearbyData objectAtIndex:[annotation.title intValue]] andType:self.dataType];
    if ([annotationView.annotation.subtitle isEqualToString:@"YES"]) {
        [annotationView wh_selectAnimation];
    }
...
//    [g_server getUser:[dict objectForKey:@"userId"] toView:self];
    ...
    vc.wh_userId       = [dict objectForKey:@"userId"];
    vc.wh_fromAddType = 6;
    vc = [vc init];
...
-(void) WH_didServerResult_WHSucces:(WH_JXConnection*)aDownload dict:(NSDictionary*)dict array:(NSArray*)array1{
    [_wait stop];
    if([aDownload.action isEqualToString:wh_act_nearbyUser]){
