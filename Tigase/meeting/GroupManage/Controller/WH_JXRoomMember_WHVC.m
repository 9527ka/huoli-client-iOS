...
//        [line release];
    ...
    if(click == check){
        ...
        UIImageView* iv;
        iv = [[UIImageView alloc] initWithFrame:CGRectMake(parent.frame.size.width - 21, (HEIGHT-12)/2.f, 14, 14)];
...
//    membHeight+=self.wh_iv.frame.size.height;
    ...
    if (self.wh_room.isShowSignIn == 1 && IS_SHOW_GROUPSIGNIN) {
        ...
        self.wh_iv = [self WH_createMiXinButton:Localized(@"New_group_sign") drawTop:NO drawBottom:NO must:NO click:@selector(groupSignInMethod) ParentView:tView];
    ...
    if(_isAdmin){
        ...
        UIImageView *topSixView = [[UIImageView alloc] initWithFrame:CGRectMake(leftRightMargin, membHeight+height, JX_SCREEN_WIDTH-leftRightMargin*2, HEIGHT*2)];
        ...
        membHeight+=self.wh_iv.frame.size.height;
        self.wh_iv = [self WH_createMiXinButton:@"进出群列表" drawTop:NO drawBottom:YES must:NO click:@selector(enterAndOutGroundList) ParentView:topSixView];
        self.wh_iv.frame = CGRectMake(0, HEIGHT, topSixView.frame.size.width, HEIGHT);
        membHeight+=self.wh_iv.frame.size.height;
        membHeight+=topBottomMargin;
    }
    
    [self.redPacketView removeFromSuperview];
    self.redPacketView = [[UIImageView alloc] initWithFrame:CGRectMake(leftRightMargin, membHeight + topBottomMargin, JX_SCREEN_WIDTH-leftRightMargin*2, HEIGHT)];
    self.redPacketView.userInteractionEnabled = YES;
    self.redPacketView.backgroundColor = [UIColor whiteColor];
    self.redPacketView.layer.cornerRadius = g_factory.cardCornerRadius;
    self.redPacketView.layer.masksToBounds = YES;
    ...
    self.wh_iv = [self WH_createMiXinButton:self.wh_room.category == 1?@"群钻石":@"群优惠券" drawTop:NO drawBottom:NO must:NO click:@selector(groupRedPacketAction) ParentView:self.redPacketView];
    ...
    if ([data.role intValue] == 1 || [data.role intValue] == 2) {
        ...
        self.wh_iv = [self WH_createMiXinButton:self.wh_room.category == 1?@"群成员钻石优惠券列表":@"群成员优惠券列表" drawTop:YES drawBottom:NO must:NO click:@selector(groupMemberRedPacketListAction) ParentView:self.redPacketView];
        self.wh_iv.frame = CGRectMake(0, HEIGHT, self.redPacketView.frame.size.width, HEIGHT);
        membHeight+=self.wh_iv.frame.size.height;
        
        self.wh_iv = [self WH_createMiXinButton:self.wh_room.category == 1?@"清除群成员钻石信息":@"清除群成员优惠券信息" drawTop:YES drawBottom:NO must:NO click:@selector(groupMemberRedPacketClearAction) ParentView:self.redPacketView];
    ...
    if ([data.role intValue] == 1 || [data.role intValue] == 2 || self.wh_room.showAllValidRedPacket) {//群主开启了也可以使用
        self.wh_iv = [self WH_createMiXinButton:self.wh_room.category == 1?@"长时间未领取钻石优惠券":@"长时间未领取优惠券" drawTop:YES drawBottom:NO must:NO click:@selector(groupMemberRedPacketUnclaimedAction) ParentView:self.redPacketView];
...
//    }
    ...
    self.topDiamondView = [[UIImageView alloc] initWithFrame:CGRectMake(leftRightMargin, membHeight + topBottomMargin, JX_SCREEN_WIDTH-leftRightMargin*2, HEIGHT)];
    self.topDiamondView.userInteractionEnabled = YES;
    self.topDiamondView.backgroundColor = [UIColor whiteColor];
    self.topDiamondView.layer.cornerRadius = g_factory.cardCornerRadius;
    self.topDiamondView.layer.masksToBounds = YES;
    ...
    UIImageView *topTwoView = [[UIImageView alloc] initWithFrame:CGRectMake(leftRightMargin, membHeight+topBottomMargin, JX_SCREEN_WIDTH-leftRightMargin*2, HEIGHT)];
    ...
    UIImageView *topThreeView = [[UIImageView alloc] initWithFrame:CGRectMake(leftRightMargin, membHeight+topBottomMargin, JX_SCREEN_WIDTH-leftRightMargin*2, HEIGHT * 2)];
    ...
    UIImageView *topFourView = [[UIImageView alloc] initWithFrame:CGRectMake(leftRightMargin, membHeight+topBottomMargin, JX_SCREEN_WIDTH-leftRightMargin*2, HEIGHT * 2)];
    ...
    UIImageView *topSevenView = [[UIImageView alloc] initWithFrame:CGRectMake(leftRightMargin, membHeight+topBottomMargin, JX_SCREEN_WIDTH-leftRightMargin*2, HEIGHT)];
    ...
    _btn.frame = CGRectMake(INSETS, membHeight+topBottomMargin, WIDTH, HEIGHT);
...
// MARK: -- 群成员优惠券列表
- (void)groupMemberRedPacketListAction {
    WH_JXGroupMemberRedPacketVC *vc = [[WH_JXGroupMemberRedPacketVC alloc] init];
    vc.room = self.wh_room;
    [g_navigation pushViewController:vc animated:YES];
}
// MARK: -- 清除群成员优惠券列表
- (void)groupMemberRedPacketClearAction {
    [UIAlertController showAlertViewWithTitle:@"清除信息" message:self.wh_room.category == 1?@"确定删除群成员发送钻石和领取钻石的数据么":@"确定删除群成员发送优惠券和领取优惠券的数据么" controller:self block:^(NSInteger buttonIndex) {
        if (buttonIndex==1) {//删除
...
- (void)groupMemberRedPacketUnclaimedAction {
    WH_JXGroupMemberRedPacketUnclaimedVC *vc = [[WH_JXGroupMemberRedPacketUnclaimedVC alloc] init];
    vc.room = self.wh_room;
    [g_navigation pushViewController:vc animated:YES];
}

// MARK: -- 群内成员钻石列表
- (void)groupDiamoundListAction {
    WH_JXGroupDiamondVC *vc = [[WH_JXGroupDiamondVC alloc] init];
    vc.title = @"成员钻石列表";
    vc.room = self.wh_room;
    [g_navigation pushViewController:vc animated:YES];
}
...
-(void)onQuitRoom{
    ...
    WH_CustomActionSheetView *share = [[WH_CustomActionSheetView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, viewH) WithTitle:Localized(@"New_are_you_want_quit_group")];
    [share showInWindowWithMode:CustomAnimationModeShare inView:nil bgAlpha:0.5 needEffectView:NO];
    __weak typeof(share) weakShare = share;
    __weak typeof(self) weakSelf = self;
    share.wh_okActionBlock = ^{
...
- (void)enterAndOutGroundList {
    WH_JXGroupEnterAndOutVC *vc = [[WH_JXGroupEnterAndOutVC alloc] init];
    vc.room = self.wh_room;
    [g_navigation pushViewController:vc animated:YES];
}
