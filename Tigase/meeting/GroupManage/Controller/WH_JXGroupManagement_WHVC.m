...
//    membHeight = CGRectGetMaxY(iv.frame);
    ...
    if (IS_SHOW_GROUPSIGNIN) {
        if (self.room.isShowSignIn == 1) {
            ...
            iv = [self WH_createMiXinButton:Localized(@"New_group_sign") supView:qqView drawTop:NO drawBottom:YES must:NO click:nil];
            ...
            iv = [self WH_createMiXinButton:Localized(@"New_group_sign") supView:qqView drawTop:NO drawBottom:NO must:NO click:nil];
...
-(WH_JXImageView*)WH_createMiXinButton:(NSString*)title supView:(UIView *)sView drawTop:(BOOL)drawTop drawBottom:(BOOL)drawBottom must:(BOOL)must click:(SEL)click {
    ...
    JXLabel* p = [[JXLabel alloc] initWithFrame:CGRectMake(15, 0, 250, HEIGHT)];
    p.text = title;
