//
//  WH_JXGroupMessagesSelectFriend_WHVC.m
//  Tigase_imChatT
//
...
#import "WH_JXSelectLabels_WHVC.h"
#import "WH_JXLabelObject.h"
#import "BMChineseSort.h"

@interface WH_JXGroupMessagesSelectFriend_WHVC () <UITextFieldDelegate, WH_JXSelectLabels_WHVCDelegate>
...
@implementation WH_JXGroupMessagesSelectFriend_WHVC
...
- (void)viewDidLoad{
    ...
    [allSelect setTitleColor:HEXCOLOR(0x333333) forState:UIControlStateNormal];
    [allSelect setTitleColor:HEXCOLOR(0x333333) forState:UIControlStateSelected];
...
//    [allSelect setBackgroundImage:[UIImage createImageWithColor:[UIColor clearColor]] forState:UIControlStateSelected];
    ...
    self.nextBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, self.wh_tableFooter.frame.size.width, 48)];
    ...
    [self.wh_tableFooter addSubview:self.nextBtn];
...
- (void)selectLabels:(UIButton *)btn {
    ...
    WH_JXSelectLabels_WHVC *vc = [[WH_JXSelectLabels_WHVC alloc] init];
    vc.delegate = self;
...
- (void)allSelect:(UIButton *)btn {
    ...
    if (btn.selected) {
        ...
        for (WH_JXUserObject *user in array) {
...
- (void)nextBtnAction:(UIButton *)btn {
    ...
    for (NSInteger i = 0; i < self.selLabelsArr.count; i ++) {
        WH_JXLabelObject *labelObj = self.selLabelsArr[i];
        ...
        for (NSInteger j = 0; j < labelUserIds.count; j ++) {
            ...
            NSString *labelUserName = [WH_JXUserObject WH_getUserNameWithUserId:labelUserId];
            BOOL flag = NO;
    ...
    WH_JXChat_WHViewController *vc = [[WH_JXChat_WHViewController alloc] init];
...
//    }
    ...
    WH_JXUserObject *user=tempArray[indexPath.row];
