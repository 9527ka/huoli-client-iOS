...
#pragma mark -- 获取群组信息
- (WH_RoomData *)getRoomDataFromRoomId:(NSString *)roomJid {
    for (NSDictionary *dic in _originGroupArray) {
        if ([dic[@"id"] isEqualToString:roomJid]) {
            WH_RoomData *data = [[WH_RoomData alloc] init];
            [data WH_getDataFromDict:dic];
            return data;
        }
    }
    return nil;
