//
//  WH_JXAudioRecorder_WHViewController.m
//  Tigase_imChatT
//
...
@interface WH_JXAudioRecorder_WHViewController ()

@end

@implementation WH_JXAudioRecorder_WHViewController{
    UIButton *_recordBtn;
...
//    _recorderNoticeLabel.text = Localized(@"WaHu_JXAudioRecorder_WHViewController_TapRecorder");
...
//    [self.view addSubview:_playBtn];
    ...
    _noticeLabel.text = [NSString stringWithFormat:@"%@\r\n%@\r\n%@\r\n%@",Localized(@"WH_AudioRecorder_RecorderTip1"),Localized(@"WH_AudioRecorder_RecorderTip2"),Localized(@"WH_AudioRecorder_RecorderTip3"),Localized(@"WH_AudioRecorder_RecorderTip4")];
...
//            _playBtn.hidden = YES;
            ...
            //_recorderBtn方块
            [_recordBtn setImage:[UIImage imageNamed:@"stopPlayAudio"] forState:UIControlStateSelected];
            _recorderNoticeLabel.text = Localized(@"WH_AudioRecorder_PauseRecorder");
        }else{
            _recordBtn.hidden = NO;
//            _playBtn.hidden = NO;
            ...
            _recorderNoticeLabel.text = Localized(@"WH_AudioRecorder_ContinueRecorder");
        }
    }
}
...
-(void)confirmAudio:(UIButton *)sender{
    ...
    if (delegate && [delegate respondsToSelector:@selector(WH_AudioRecorderDidFinish:TimeLen:)] ) {
        [delegate WH_AudioRecorderDidFinish:_lastRecordFile TimeLen:_timeCount];
    }
    [super actionQuit];
}
