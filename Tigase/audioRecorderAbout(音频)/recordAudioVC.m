...
-(void)build{
    _iv = [[WH_JXImageView alloc] initWithFrame:CGRectMake(0, 0, JX_SCREEN_WIDTH, JX_SCREEN_HEIGHT-200)];
...
//    [bottom release];
    ...
    for(int i=0;i<n;i++){
        WH_JXImageView* iv = [[WH_JXImageView alloc]initWithFrame:CGRectMake(i*70+5, 2, 65, 65)];
        iv.wh_delegate = self;
...
//    NSURL* url = [[NSURL alloc] initFileURLWithPath:_mixRecorder.outputAudioFile];
    _player.wh_audioFile = _mixRecorder.outputAudioFile;
}
...
-(void)doShowBtn{
        if(_btnRecord.wh_selected){
            _volume.hidden  = NO;
            _input.hidden   = NO;
            _btnBack.hidden = YES;
            _btnDel.hidden  = NO;
            _btnPlay.hidden = YES;
            _btnEnter.wh_enabled = NO;
        }
        else{
            _volume.hidden  = YES;
            _input.hidden   = YES;
            ...
            _btnDel.hidden  = YES;
            _btnPlay.hidden = NO;
            _btnEnter.wh_enabled = YES;
        }
}

-(void)onRecord{
    _btnRecord.wh_selected = !_btnRecord.wh_selected;
    [self doShowBtn];
    if(!_btnRecord.wh_selected){
        _btnRecord.image = [UIImage imageNamed:@"publish_toolbar_record_normal"];
        [self freeTimer];
