...
@implementation WH_Gif_GHViewController
@synthesize delegate=_delegate,wh_faceArray,imageArray;
...
- (id)initWithFrame:(CGRect)frame {
    ...
    wh_faceArray  = [[NSMutableArray alloc] init];
    margin = 18;
    ...
    for (NSString *aPath in contentOfFolder) {
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:Path isDirectory:&isDir] && !isDir)
        {
            if( [ext isEqualToString:@"gif"] ){
                WH_SCGIFImageView* iv = [[WH_SCGIFImageView alloc] initWithGIFFile:Path];
                [imageArray addObject:[iv getFrameAsImageAtIndex:0]];
                [wh_faceArray addObject:[Path lastPathComponent]];
//                [iv release];
            }
        }
    }

    int n = fmod([wh_faceArray count], (tempN * 2));
    maxPage = [wh_faceArray count]/(tempN*2);
    if(n != 0)
        maxPage++;
}
...
{
    NSString* s = [wh_faceArray objectAtIndex:sender.tag];
