...
@interface WH_JXEmoji () <WH_JXActionSheet_WHVCDelegate>

@property (nonatomic, strong) WH_JXActionSheet_WHVC *actionVC;

@end

@implementation WH_JXEmoji
...
- (BOOL)label:(WH_JXEmoji *)label didBeginTouch:(UITouch *)touch onCharacterAtIndex:(CFIndex)charIndex {
    
    [self highlightLinksWithIndex:charIndex];
    return YES;
}

- (BOOL)label:(WH_JXEmoji *)label didMoveTouch:(UITouch *)touch onCharacterAtIndex:(CFIndex)charIndex {
    
    [self highlightLinksWithIndex:charIndex];
    return YES;
}
//这里对文本的电话号码处理
- (BOOL)label:(WH_JXEmoji *)label didEndTouch:(UITouch *)touch onCharacterAtIndex:(CFIndex)charIndex {
    ...
    for (NSTextCheckingResult *match in self.matches) {
        ...
        if ([match resultType] == NSTextCheckingTypePhoneNumber) {
            ...
            if ([self isIndex:charIndex inRange:matchRange]) {
                ...
                self.actionVC = [[WH_JXActionSheet_WHVC alloc] initWithImages:@[] names:@[Localized(@"JX_Copy"),Localized(@"JXEmoji_CallPhone")]];
                self.actionVC.delegate = self;
                self.actionVC.wh_tag = 1;
...
//                self.actionVC = [[WH_JXActionSheet_WHVC alloc] initWithImages:@[] names:@[Localized(@"JX_Copy"),Localized(@"JXEmoji_OpenUrl")]];
//                self.actionVC.delegate = self;
//                self.actionVC.tag = 2;
//                [g_App.window addSubview:self.actionVC.view];
...
- (BOOL)label:(WH_JXEmoji *)label didCancelTouch:(UITouch *)touch {
