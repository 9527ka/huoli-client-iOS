...
@interface WH_JXSystemImage2_WHCell ()
...
@implementation WH_JXSystemImage2_WHCell
...
-(void)setCellData{
    ...
    CGSize size = [_title.text boundingRectWithSize:CGSizeMake(_title.frame.size.width, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:sysFontWithSize(15.0)} context:nil].size;
...
- (void) creatSubViews:(NSArray *)array{
    ...
    for (NSInteger i = 1; i < array.count; i ++) {
        ...
        label.font = sysFontWithSize(14.0);
...
- (void) subViewsBtnAction:(UIButton *) btn {
    NSArray *content = [self.msg.content mj_JSONObject];
