...
//    _headImageView.didTouch = @selector(WH_headImageDidTouch);
    ...
    _roleLabel = [[UILabel alloc] initWithFrame:CGRectMake(JX_SCREEN_WIDTH - 130, 0, 120, CELL_HEIGHT)];
...
- (void)setData:(memberData *)data {
    ...
    if ([mData2.onLineState integerValue] == 0) {
        ...
        if (mData2.offlineTime == 0) { //机器人
            //在线
            [_stateLabe setText:Localized(@"JX_OnLine")];
            ...
            if (self.isMeCreatGroup) {
                NSString *data = [self dateTimeDifferenceWithStartTime:[NSNumber numberWithLongLong:timeIn]];
                [_stateLabe setText:[NSString stringWithFormat:@"%@%@" ,data,Localized(@"JX_OnLine")]];
            } else {
                [_stateLabe setText:@""];
            }
