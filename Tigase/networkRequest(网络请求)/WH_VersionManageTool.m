...
//        configuration.appID = @"1259280364";
    ...
    if ([p objectForKey:@"minTransferAmount"]) {
        self.minTransferAmount = [p objectForKey:@"minTransferAmount"];
    }
    if ([p objectForKey:@"maxTransferAmount"]) {
        self.maxTransferAmount = [p objectForKey:@"maxTransferAmount"];
    }
...
-(void)didReceive:(NSDictionary*)dict{
    @try {
        ...
        NSString *apiUrls = [NSString stringWithFormat:@"%@",[dict objectForKey:@"apiUrl"]];
        if(apiUrls.length > 0){
...
//    NSString* defApiUrl = @"http://38.34.187.16:8092/config";
    //业务基地址
