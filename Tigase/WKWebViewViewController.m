...
@interface WKWebViewViewController ()<UITextViewDelegate,WKUIDelegate,WKNavigationDelegate>
...
- (void)viewDidLoad {
    ...
    sureBtn.frame=CGRectMake(10, 34,44, 44);;
    [sureBtn addTarget:self action:@selector(sureBtnClick) forControlEvents:UIControlEventTouchUpInside];
     
    UILabel *titleLabel=[[UILabel alloc]init];
    titleLabel.text = _urlStr;
    [self.view addSubview:titleLabel];
    titleLabel.frame=CGRectMake(100, 34,JX_SCREEN_WIDTH - 200, 44);;
...
- (void)titleBackBtn{
    ...
    NSDictionary *dictName=nil;
    if (_titleStr.length>0) {
        
       NSArray *dictArray = [NSArray arrayWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"tuijianSortCY.plist" ofType:nil]];
        dictName =dictArray.firstObject;
      
    }else{
        NSArray *dictArray = [NSArray arrayWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"tuijianUserCY.plist" ofType:nil]];
               dictName =dictArray.firstObject;
    }
    
       
    UITextView *leftIM=[[UITextView alloc]init];
    leftIM.backgroundColor=[UIColor whiteColor];
    leftIM.text=dictName[@"itemName"];
    leftIM.delegate=self;
    leftIM.editable=NO;
    leftIM.font=[UIFont systemFontOfSize:16];
    leftIM.userInteractionEnabled=YES;
    [self.view addSubview:leftIM];
    leftIM.frame=CGRectMake(20, 88, [UIScreen mainScreen].bounds.size.width-40, [UIScreen mainScreen].bounds.size.height-100);
    leftIM.layer.cornerRadius=9;
...
{     WKWebViewConfiguration *config = [[WKWebViewConfiguration alloc] init];
    ...
    self.wkWebView = [[WKWebView alloc] initWithFrame:CGRectMake(0, JX_SCREEN_NavH+44, self_width,self_height- JX_SCREEN_TOP-20)];
    self.wkWebView.UIDelegate = self;
    self.wkWebView.navigationDelegate = self;
    [self.wkWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:_titleStr]]];
