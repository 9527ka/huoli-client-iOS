...
@interface WH_selectProvince_WHVC ()<CLLocationManagerDelegate>
@property(nonatomic,strong)CLLocationManager *locationManager;
@property(nonatomic,strong)NSMutableString * cityName;
@end

@implementation WH_selectProvince_WHVC
@synthesize showCity;
@synthesize selected;
@synthesize delegate;
...
- (id)init
{
    ...
    if (self) {
        ...
        self.title =Localized(@"WaHu_selectProvince_WaHuVC_SelProvince");
        //self.view.frame = CGRectMake(0, 0, JX_SCREEN_WIDTH, JX_SCREEN_HEIGHT);
        [self WH_createHeadAndFoot];
        self.wh_isShowFooterPull = NO;
        self.wh_isShowHeaderPull = NO;
        self.cityName = [[NSMutableString alloc]initWithString:Localized(@"WaHu_selectProvince_WaHuVC_Locationing")];
        if(self.parentId<=0)
            self.parentId = 1;
        ...
        [_array setObject:Localized(@"WaHu_selectProvince_WaHuVC_Beijing") forKey:@"110100"];//110000,110100
        [_array setObject:Localized(@"WaHu_selectProvince_WaHuVC_Shanghai") forKey:@"310100"];//310000,310100
        [_array setObject:Localized(@"WaHu_selectProvince_WaHuVC_Guangzhou") forKey:@"440100"];//440000,440100
        [_array setObject:Localized(@"WaHu_selectProvince_WaHuVC_Shenzhen") forKey:@"440300"];//440000,440300
    }
    return self;
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    switch (section) {
        case 0:
            s = Localized(@"WaHu_selectProvince_WaHuVC_NowLocation");
            break;
        case 1:
            s = Localized(@"WaHu_selectProvince_WaHuVC_HotCity");
            break;
        case 2:
            s = Localized(@"WaHu_selectProvince_WaHuVC_SelCity");
            break;
    }
    p.text = s;
...
//        //NSLog(@"%@",g_constant.cityN);
        ...
        //县
        NSString * cityNameX = [NSString stringWithFormat:@"%@%@",self.cityName,Localized(@"WaHu_selectProvince_WaHuVC_County")];
        //区
        NSString * cityNameQ = [NSString stringWithFormat:@"%@%@",self.cityName,Localized(@"WaHu_selectProvince_WaHuVC_Area")];
        ...
        if ([g_constant.cityN valueForKey:self.cityName]) {
            ...
            if ([self.cityName isEqualToString:Localized(@"WaHu_selectProvince_WaHuVC_Locationing")]|| [self.cityName isEqualToString:@""]) {
                self.selValue = Localized(@"WaHu_selectProvince_WaHuVC_LocationFiled");
            }
            self.cityId = 1;
            self.provinceId = 1;
        }
//        self.provinceId = 1;
    ...
    if(indexPath.section==2){
        ...
        if(self.showCity){
            WH_selectCity_WHVC* vc = [WH_selectCity_WHVC alloc];
...
-(void)doSelect:(WH_selectCity_WHVC*)sender{
    [self actionQuit];
