//
//  WH_ImageBrowser_WHViewController.m
//  ImageBrowser
//
...
@interface WH_ImageBrowser_WHViewController ()<UIScrollViewDelegate,WH_PhotoViewDelegate,WH_JXActionSheet_WHVCDelegate,KKImageEditorDelegate,UINavigationControllerDelegate>{
...
/** 记录当前的图片显示视图 */
@property(nonatomic,strong) WH_PhotoView *WH_PhotoView;

@end
static WH_ImageBrowser_WHViewController *shared;
@implementation WH_ImageBrowser_WHViewController

//+ (WH_ImageBrowser_WHViewController*)sharedInstance{
//    static dispatch_once_t onceToken;
//    dispatch_once(&onceToken, ^{
//        shared = [[WH_ImageBrowser_WHViewController alloc]init];
//    });
//    return shared;
//}
- (void)dealloc{
...
-(void)loadPhote:(NSInteger)index{
    ...
    id currentWH_PhotoView = [_subViewArray objectAtIndex:index];
    if (![currentWH_PhotoView isKindOfClass:[WH_PhotoView class]]) {
        //url数组或图片数组
        ...
        if ([[self.imagesArray firstObject] isKindOfClass:[UIImage class]]) {
            WH_PhotoView *photoV = [[WH_PhotoView alloc] initWithFrame:frame withPhotoImage:[self.imagesArray objectAtIndex:index]];
            photoV.delegate = self;
            [self.scrollView insertSubview:photoV atIndex:0];
            [_subViewArray replaceObjectAtIndex:index withObject:photoV];
            self.WH_PhotoView=photoV;
        }else if ([[self.imagesArray firstObject] isKindOfClass:[NSString class]]){
            WH_PhotoView *photoV = [[WH_PhotoView alloc] initWithFrame:frame withPhotoUrl:[self.imagesArray objectAtIndex:index]];
            photoV.delegate = self;
            [self.scrollView insertSubview:photoV atIndex:0];
            [_subViewArray replaceObjectAtIndex:index withObject:photoV];
            self.WH_PhotoView=photoV;
...
+(void)show:(UIViewController *)handleVC delegate:(id)delegate type:(PhotoBroswerVCType)type contentArray:(NSMutableArray *)contentArray index:(NSUInteger)index imagesBlock:(NSArray *(^)())imagesBlock{
    ...
    WH_ImageBrowser_WHViewController *imgBrowserVC = [[WH_ImageBrowser_WHViewController alloc] init];
...
-(void)hideScanImageVC{
    if (self.delegate && [self.delegate respondsToSelector:@selector(WH_dismissImageBrowserVC)]) {
        [self.delegate WH_dismissImageBrowserVC];
    }
...
//    self.pageControl.currentPage = page;
    ...
    for (UIView *view in scrollView.subviews) {
        if ([view isKindOfClass:[WH_PhotoView class]]) {
            WH_PhotoView *photoV=(WH_PhotoView *)[_subViewArray objectAtIndex:page];
            if (photoV!=self.WH_PhotoView) {
                [self.WH_PhotoView.scrollView setZoomScale:1.0 animated:YES];
                self.WH_PhotoView=photoV;
            }
        }
    }
...
#pragma mark - WH_PhotoViewDelegate
...
//    [tagView removeFromSuperview];
    ...
    if (index == 0) {
        ...
        WH_KKImageEditor_WHViewController *editor = [[WH_KKImageEditor_WHViewController alloc] initWithImage:self.WH_PhotoView.imageView.image delegate:self];
...
{
    self.WH_PhotoView.imageView.image = image;
    [self setupActionSheet];
}


- (void) distinguishQRCode {
    UIImageView*tempImageView=(UIImageView*)self.WH_PhotoView.imageView;
    ...
    if(tempImageView.image){
        ...
        if ([self.delegate respondsToSelector:@selector(WH_imageBrowserVCQRCodeAction:)]) {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [self hideScanImageVC];
                [self.delegate WH_imageBrowserVCQRCodeAction:scannedResult];
            });
        }
        
//        [self QRCodeAction:scannedResult];
//        UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"扫描结果" message:scannedResult delegate:nil cancelButtonTitle:Localized(@"JX_Confirm") otherButtonTitles:nil, nil];
//
//        [alertView show];
